...
public class DataInputStreamThreadTestShared extends InputStreamThreadTestShared {
    public InputStream getTestedInputStream() throws IOException {
        byte[] arr = new byte[THREADS * READS * THREADS];
        for(int i = 0; i < arr.length; ++i) {
            arr[i] = (byte) ('a' + (i % THREADS));
        }
        return new DataInputStream(new ByteArrayInputStream(arr));
    }
