 ...
 */
public class EventTest001 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT001.EventDebuggee001";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT001/EventDebuggee001_Thread;";
     ...
     * 'METHOD_EXIT' events are received.
     */
    public Result  testEvent001() {

        logWriter.println("==> testEvent001: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee001.THREAD_NAME_PATTERN + i;
        ...
        if ( methodEntryRequestID != NO_REQUEST_ID ) {
            ...
            for (int i=0; i < startedThreadsNumber; i++) {
                ...
                if ( eventKind != JDWPConstants.EventKind.METHOD_ENTRY ) {
                    boolean toPrint = limitedPrintln("## FAILURE: Unexpected event for thread name = " +
                            startedThreadsNames[i]);
                    if ( toPrint ) {
                ...
                if ( ! expectedMethodName.equals(methodName) ) {
                    boolean toPrint = 
                        limitedPrintln("## FAILURE: Unexpected method name for 'METHOD_ENTRY' event");
                    if ( toPrint ) {
        ...
        if ( methodExitRequestID != NO_REQUEST_ID ) {
            ...
            for (int i=0; i < startedThreadsNumber; i++) {
                ...
                if ( ! expectedMethodName.equals(methodName) ) {
                    boolean toPrn = 
                        limitedPrintln("## FAILURE: Unexpected method name for 'METHOD_EXIT' event");
                    if ( toPrn) {
                        logWriter.println("##          Received method name = " + methodName);
                        logWriter.println("##          Expected method name = " + expectedMethodName);
                    }
            ...
            if ( receivingMethodExitStatus == FAILURE ) {
                ...
                terminateDebuggee(FAILURE, "MARKER_11.2");
        ...
        logWriter.println("==> testEvent001: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent001: FAILED");
            return failed("testEvent001:");
        } else {
            logWriter.println("==> testEvent001: OK");
            return passed("testEvent001: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent001: FAILED");
    return failed("==> testEvent001: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest001().test(args));
	}
}
