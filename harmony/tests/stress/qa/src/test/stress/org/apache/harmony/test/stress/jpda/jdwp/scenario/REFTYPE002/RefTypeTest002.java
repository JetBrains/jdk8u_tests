 ...
 * AllClasses command. Then test creates memory stressing by loading in memory
 * very big array. Then checks if all RefTypeIDs have expected SourceFile and
 * signatures.
 *  
 */
public class RefTypeTest002 extends StressTestCase {

    public final static String DEBUGGEE_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.REFTYPE002.RefTypeDebuggee002";
    ...
    /**
     * This tests case exercises the JDWP agent under ReferenceTypeIDs
     * stressing. First, test loads very big number of classes on debuggee side,
     * sends <code>VirtualMachine.AllClasses</code> command, saves and checks
     * returned RefTypeIDs. Then, test loads vast number of new classes, sends
     ...
     * first AllClasses command. Then test creates memory stressing by loading in memory very big array. Then checks if all RefTypeIDs have expected
     * SourceFile and signatures.
     *  
     */
    public Result testRefType002() {
        int testCaseStatus = SUCCESS;
        logWriter.println("==> testRefType002: START (" + new Date() + ")...");
...
try {
        ...
        if ( saveRefTypeIDsStatus ) {
            if (!checkClasses(refTypesArray, signaturesArray, tagsArray, refTypesArray.getLength())) {
                logWriter.println("##FAILURE: during checkClasses");
                testCaseStatus = FAILURE;
            }
        }
        ...
        if (!SIGNAL_READY_04.equals(debuggeeSignal)) {
            ...
            return failed("## FAILURE while receiving 'SIGNAL_READY_04' Signal from debuggee");
        }
        if ( saveRefTypeIDsStatus ) {
            if (!checkClasses(refTypesArray, signaturesArray, tagsArray, refTypesArray.getLength())) {
                logWriter.println("##FAILURE: during checkClasses");
        ...
        logWriter.println("==> Checking debuggee class..");
        reply = debuggeeWrapper.vmMirror.getClassBySignature(DEBUGGEE_SIGNATURE);
        if (checkReplyForError(reply, JDWPConstants.Error.NONE,
          "VirtualMachine::ClassesBySignature command")) {
            terminateDebuggee(FAILURE, "MARKER_13");
            return failed("## VirtualMachine::ClassesBySignature command ERROR");
        }
        ...
        for (int i = 0; i < numberOfClasses; i++) {
            ...
            if (!source.equals("RefTypeDebuggee002.java")) {
                testCaseStatus = FAILURE;
                logWriter.println("#FAILURE: returned sourceFile for debuggee class is " + source + " instead of RefTypeDebuggee002.java");
            }
            String signature = debuggeeWrapper.vmMirror.getClassSignature(refType);
            if (!signature.equals(DEBUGGEE_SIGNATURE)) {
                testCaseStatus = FAILURE;
                logWriter.println("#FAILURE: returned signature for debuggee class: " + signature + " instead of " +DEBUGGEE_SIGNATURE);
            }
        }
        
        if ( saveRefTypeIDsStatus ) {
            if (!checkNewClasses(refTypesArray, signaturesArray, tagsArray, 2)) {
                logWriter.println("##FAILURE: during checking new classes");
                testCaseStatus = FAILURE;
            }
        }
        
        long testRunTimeMlsec = System.currentTimeMillis() - testStartTimeMlsec;
        logWriter.println("==> testRefType002: running time(mlsecs) = "
                + testRunTimeMlsec);
        if (testCaseStatus == FAILURE) {
            logWriter.println("==> testRefType002: FAILED");
            return failed("testRefType002:");
        } else {
            logWriter.println("==> testRefType002: OK");
            return passed("testRefType002: OK");
        }
    ...
    logWriter.println("==> testRefType002: FAILED");
    return failed("==> testRefType002: Unexpected Exception! ");
}
    }

    public static void main(String[] args) {
        System.exit(new RefTypeTest002().test(args));
    }
}