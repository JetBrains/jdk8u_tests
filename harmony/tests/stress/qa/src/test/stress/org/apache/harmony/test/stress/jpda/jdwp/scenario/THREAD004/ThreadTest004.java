...
/**
  ...
  * ClassType.InvokeMethod - each command in the separate thread
  * and under memory stress in debuggee.
  * The test expects that all commands result in expected returned value. 
 */
public class ThreadTest004 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD004.ThreadDebuggee004";
    ...
    /**
      ...
      * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread004() {
        logWriter.println("==> testThread004: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee004.METHOD_TO_INVOKE_NAME);
        ...
        measurableCodeRunningTimeMlsec = System.currentTimeMillis() - measurableCodeStartTimeMlsec;
        logWriter.println
        ("==> Time(mlsecs) of waiting for debuggee to create memory stress = " + measurableCodeRunningTimeMlsec);
        ...
        logWriter.println("==> testThread004: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread004: FAILED");
            return failed("testThread004:");
        } else {
            logWriter.println("==> testThread004: OK");
            return passed("testThread004: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread004: FAILED");
    return failed("==> testThread004: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest004().test(args));
	}
}
