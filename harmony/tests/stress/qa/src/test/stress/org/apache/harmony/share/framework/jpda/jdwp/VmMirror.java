...
import org.apache.harmony.share.framework.jpda.Breakpoint;
...
import org.apache.harmony.share.framework.jpda.jdwp.Capabilities;
...
import org.apache.harmony.share.framework.jpda.jdwp.TypesLengths;
...
/**
 * This class provides convenient way for communicating with debuggee VM 
 * using JDWP packets.
 * <p>
 * Most methods can throw ReplyErrorCodeException if error occured in execuiton
 * of corresponding JDWP command or TestErrorException if any other error occured.
 */
public class VmMirror {
    ...
    /** Transport used to sent and recieve packets. */
    ...
    /**
     * Checks error code of given reply packet and throws ReplyErrorCodeException 
     * if any error detected.
     * 
     * @param reply reply packet to check
...
/*        // Get Class reference ID
        ...
        // Fill location
        Location location = new Location(typeTag, typeID, methodID, breakpoint.index);

        // Set breakpoint
        return setBreakpoint(location);*/
        return setBreakpoint(typeTag, breakpoint, JDWPConstants.SuspendPolicy.ALL);
    }
     ...
     * @param typeTag
     * @param breakpoint
     * @param suspendPolicy Suspend policy for a breakpoint being created
     * @return ReplyPacket for corresponding command
     */
    public ReplyPacket setBreakpoint(byte typeTag, Breakpoint breakpoint, byte suspendPolicy) {
        // Get Class reference ID
        long typeID = getTypeID(breakpoint.className, typeTag);
...
/*        // Prepare corresponding event
     ...
     * @param breakpoint
     * @param suspendPolicy Suspend policy for a breakpoint being created
     * @param count Limit the requested event to be reported at most once after a given number of occurrences
     * @return ReplyPacket for corresponding command
     */
     ...
     * @param classID  id of class with required method  
     * @param methodName  name of required method
    ...
    public long setBreakpointAtMethodBegin(long classID, String methodName){
        ...
        if(lineTableReply.getErrorCode() != JDWPConstants.Error.NONE){
            throw new TestErrorException("Command getLineTable returned error code: "+lineTableReply.getErrorCode()+" - "+JDWPConstants.Error.getName(lineTableReply.getErrorCode()));
        }
     ...
     * @param  requestID id of request for breakpoint
    ...
    public long waitForBreakpoint(long requestID) {
        ...
        int numberOfEvents = event.getNextValueAsInt();
                       
        long breakpointThreadID = 0;
        ParsedEvent[] eventParsed = ParsedEvent.parseEventPacket(event);

        if (eventParsed.length != 1) {
            throw new TestErrorException("Received "+eventParsed.length+" events instead of 1 BREAKPOINT_EVENT"); 
        }
        ...
        if (eventParsed[0].getEventKind() == JDWPConstants.EventKind.BREAKPOINT) {
           ...
           throw new TestErrorException("Kind of received event is not BREAKPOINT_EVENT: "+eventParsed[0].getEventKind());
            
        }
        
        if (eventParsed[0].getRequestID() != requestID) {
            throw new TestErrorException("Received BREAKPOINT_EVENT with another requestID: "+eventParsed[0].getRequestID());
        }
     ...
     * 
     * @param requestID for given breakpoint
     * @return ReplyPacket for corresponding command
     */
    ...
    public long getTypeID(String typeSignature, byte classTypeTag) {
        ...
        // Get referenceTypeID from recieved packet
        ...
        for (int i = 0; i < classes; i++) {
            ...
            if (refTypeTag == classTypeTag) {
                ...
                status = classReference.getNextValueAsInt();
                break;
            } else {
     ...
     * 
     * @param interfaceSignature interface signature
     * @return received ClassID
     */
     ...
     * 
     * @param arraySignature array signature
     ...
     * 
     * @param request ReplyPacket with RequestID 
     ...
     * 
     * @param classID ClassID to find field
     ...
     * 
     * @param request ReplyPacket for request
     * @param field field name to get ID for
     * @return received FieldID
     */
     ...
     * 
     * @param classID class to find method
     * @param methodName method name
     * @return receuved MethodID
     */
    public long getMethodID(long classID, String methodName) {
        ...
        // Get methodID from recieved packet
        ...
        for (int i = 0; i < declared; i++) {
            ...
            if (methodName.equals(method)) {
                ...
                methodSignature = reply.getNextValueAsString();
    ...
    /**
     * Sets ClassPrepare event request for given class name patern.
     * 
     * @param classRegexp
    ...
    /**
     * Sets exception event request for given exceptiuon class signature.
     * 
     ...
     * @return ReplyPacket for corresponding command
     */
    public ReplyPacket setException(String exceptionSignature, 
     ...
     * @return ReplyPacket for corresponding command
     */
    public ReplyPacket setException(long exceptionID, 
                                boolean caught, boolean uncaught) {
        // Prepare corresponding event
    ...
    /**
     * Sets exception event request for given exceptiuon class signature.
     * @param exceptionSignature exception signature.
     * @param caught is exception caught
     * @param uncaught is exception uncaught
     * @param count Limit the requested event to be reported at most once after a given number of occurrences
     * @return ReplyPacket for corresponding command
     */
     ...
     * @param classRegexp class name pattern or null for no pattern
     * 
     * @return ReplyPacket for corresponding command
     */
    ...
    /**
     * Sets METHOD_EXIT event request for specified class name pattern.
     * @param classRegexp classRegexp class name pattern or null for no pattern
     * @param count Limit the requested event to be reported at most once after a given number of occurrences
     * @return ReplyPacket for corresponding command
     */
     ...
     */
    public ReplyPacket setFieldAccess(String classSignature,
     ...
     * 
     * @param classRegexp      
     *		Disallowed class patterns. Matches are limited to exact matches of the given class pattern 
     * 		and matches of patterns that begin or end with '*'; for example, "*.Foo" or "java.*".
     * @param stepSize
     * @param stepDepth
     * @return ReplyPacket for setting request.
     */
    public ReplyPacket setStep(String[] classRegexp, long threadID, int stepSize, int stepDepth) {
        // Prepare corresponding event
        byte eventKind = JDWPConstants.EventKind.SINGLE_STEP;
        byte suspendPolicy = JDWPConstants.SuspendPolicy.ALL;
    ...
    /**
     * Sets VM_DEATH event request.
     * 
     * @return ReplyPacket for corresponding command
     */
    public ReplyPacket setVMDeath() {
        /*
         * TODO: implement
         */
        return null;
     ...
     * All thrown exceptions are wrapped into TestErrorException.
     * Consider using checkReply() for checking error code in reply packet.
     * 
     * @param command
     ...
     *            Command packet to be sent
     * @param timeout
     *            Timeout in milleseconds for waiting reply packet
     * @return received ReplyPacket
     ...
     * method. Default timeout is used as time limit for waiting. 
     ...
     *             if timeout exceeded
     */
    public ReplyPacket receiveReply(int commandId)
     ...
     * Throws TestErrorException if receved event is not of expected kind 
     * or not a single event in the received event set.
     * 
     * @param type
    ...
    /**
     * Sets established connection channel to be used with this VmMirror
     * and starts reading packets.
     * @param connection
     ...
     * @param threadID The thread object ID. 
    ...
    /**
     * Returns a list containinig all frames of a certain thread
     ...
     * @param threadID ID of the thread whose frames to obtain
     ...
     * @return ReplyPacket for corresponding command
     */
    public final ReplyPacket getThreadFrames(long threadID, int startIndex, int length) {
        CommandPacket command = new CommandPacket(
            JDWPCommands.ThreadReferenceCommandSet.CommandSetID,
     ...
     * @return A list containing all variables (arguments and locals) declared within the method. 
     */
     ...
     * @param frame Frame whose variables to get
     ...
     * @param classID The class ID whose superclass ID is to get 
     * @return The superclass ID (null if the class ID for java.lang.Object is specified).  
     */
    ...
    public final ReplyPacket getLineTable(long refType, long methodID) {
        ...
        command.setNextValueAsMethodID(methodID);
        //ReplyPacket reply = debuggeeWrapper.vmMirror.checkReply(debuggeeWrapper.vmMirror.performCommand(command));
        // it is impossible to obtain line table information from native methods, so reply checking is not performed 
        ReplyPacket reply = performCommand(command);
        if (reply.getErrorCode() != JDWPConstants.Error.NONE) {
     ...
     * @param objectID The object ID  
     * @param fieldIDs IDs of fields to get
     ...
     * @param threadID The frame's thread ID
     ...
     * @param classID The reference type ID
    ...
    public final List getAllFields(long classID) {
        ...
        for (int i = 0; i < fieldsCount; i++) {
            Field field = new Field(reply.getNextValueAsFieldID(), classID, reply.getNextValueAsString(), reply.getNextValueAsString(), reply.getNextValueAsInt());
            fields.add(field);
        }
    ...
    public final String getMethodSignature(long classID, long methodID) {
        ...
        command.setNextValueAsReferenceTypeID(classID);
        ReplyPacket reply = checkReply(performCommand(command));
        int modBits, methods = reply.getNextValueAsInt();
        String value = null;
     ...
     * @param objectID The String object ID.   
     ...
     * @param classID The class object ID.
     * @param methodID The method ID.
     * @param codeIndex The line code index. 
     ...
     */
    public final long getLineCodeIndex(long classID, long methodID, int lineNumber) {
        ReplyPacket reply = getLineTable(classID, methodID);
        if (reply.getErrorCode() != JDWPConstants.Error.NONE) {
            return -1L;
        }
     ...
     * @param frame The frame whose visible local variables to retrieve.
     * @return A list of Variable objects representing each visible local variable within the given frame.
     */
    public final List getLocalVars(Frame frame) {
        List vars = getVariableTable(frame.getLocation().classID, frame.getLocation().methodID);
        if (vars == null) {            
            return null;
        }
        
        // All variables that are not visible from within current frame must be removed from the list
        long frameCodeIndex = frame.getLocation().index;
        for (int i = 0; i < vars.size(); i++) {
            Variable var = (Variable)vars.toArray()[i];
            long varCodeIndex = var.getCodeIndex(); 
            if (varCodeIndex > frameCodeIndex || (frameCodeIndex >= varCodeIndex + var.getLength())) {
                vars.remove(i);
                --i;
                continue;
            }
     ...
     * @param vars An array of Variable objects whose values to set 
     * @param values An array of Value objects to set
     */
     ...
     * @param objectID The object ID.
     * @param fieldIDs An array of fields IDs
     * @param values An array of Value objects representing each value to set
     */
    public final void setInstanceFieldsValues(long objectID, long[] fieldIDs, Value[] values) {
     ...
     * @param arrayID The array object ID.
     * @param firstIndex The first index to set. 
     * @param values An array of Value objects representing each value to set.
     */
    public final void setArrayValues(long arrayID, int firstIndex, Value[] values) {
        CommandPacket command = new CommandPacket(
            JDWPCommands.ArrayReferenceCommandSet.CommandSetID,
     ...
     * @param values An array of Value objects representing each value to set
     */
    public final void setStaticFieldsValues(long classID, long[] fieldIDs, Value[] values) {
        if (fieldIDs.length != values.length) {
            throw new TestErrorException("Number of fields doesn't correspond to number of their values");
        }
    ...
    public final ReplyPacket invokeInstanceMethod(long objectID, long threadID, String methodName, Value[] args, int options) {
    ...
    public final ReplyPacket invokeStaticMethod(long classID, long threadID, String methodName, Value[] args, int options) {
        long methodID = getMethodID(classID, methodName);
        CommandPacket command = new CommandPacket(
            JDWPCommands.ClassTypeCommandSet.CommandSetID,
