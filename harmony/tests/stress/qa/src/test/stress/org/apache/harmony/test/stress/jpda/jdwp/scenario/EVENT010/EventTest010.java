 ...
 */
public class EventTest010 extends StressTestCase {
    
    final int REQUESTS_NUMBER_FOR_EACH_THREAD = EVENT010_REQUESTS_NUMBER_FOR_EACH_THREAD;

    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT010.EventDebuggee010";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT010/EventDebuggee010_Thread;";
     ...
     * 'METHOD_EXIT' events are received.
     */
    public Result  testEvent010() {

        logWriter.println("==> testEvent010: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee010.THREAD_NAME_PATTERN + i;
        ...
        if ( ! SIGNAL_READY_02.equals(debuggeeSignal) ) {
            terminateDebuggee(FAILURE, "MARKER_11.0.05");
            return failed
        ...
        logWriter.println("==> testEvent010: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent010: FAILED");
            return failed("testEvent010:");
        } else {
            logWriter.println("==> testEvent010: OK");
            return passed("testEvent010: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent010: FAILED");
    return failed("==> testEvent010: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest010().test(args));
	}
}
