 ...
 */
public class MemoryTest005 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.MEMORY005.MemoryDebuggee005";
     ...
     * conditions that OutOfMemory happens in debuggee.
     */
    public Result  testMemory005() {

        logWriter.println("==> testMemory005: START (" + new Date() + ")...");
...
try {
        ...
        packet.setNextValueAsInt(MemoryDebuggee005.SMALL_ARRAY_LENGTH); // The number of components to retrieve
        ...
        logWriter.println("\n");
        logWriter.println
        ("==> Wait for debuggee to create another array with very big number of objects " +
                "until OutOfMemory...");
        ...
        String bigObjectArrayForOutOfMemoryFieldName = "bigObjectArrayForOutOfMemory";
        long bigObjectArrayForOutOfMemoryID = 
            getArraIDForStaticArrayField(debuggeeRefTypeID, bigObjectArrayForOutOfMemoryFieldName);
        if ( bigObjectArrayForOutOfMemoryID == BAD_ARRAY_ID ) {
            logWriter.println("## FAILURE: Can NOT get bigObjectArrayForOutOfMemoryID");
            terminateDebuggee(FAILURE, "MARKER_10");
            return failed("## Can NOT get bigObjectArrayForOutOfMemoryID");
        }
        printlnForDebug("bigObjectArrayForOutOfMemoryID = " + bigObjectArrayForOutOfMemoryID);

        logWriter.println("==> Get bigObjectArray actual length...");
        bigObjectArrayLength =
            getIntValueForStaticField(debuggeeRefTypeID, "bigObjectArrayForOutOfMemoryActualLength");
        if ( bigObjectArrayLength == BAD_INT_VALUE ) {
            logWriter.println("## FAILURE: Can NOT get bigObjectArrayForOutOfMemoryLength");
            terminateDebuggee(FAILURE, "MARKER_11");
            return failed("## Can NOT get bigObjectArrayForOutOfMemoryLength");
        }
        logWriter.println("==> bigObjectArrayForOutOfMemoryLength = " + bigObjectArrayLength);
        
        logWriter.println("\n");
        logWriter.println("==> Send ArrayReference::GetValues command for bigObjectArrayForOutOfMemory...");
        packet = new CommandPacket(
                JDWPCommands.ArrayReferenceCommandSet.CommandSetID,
                JDWPCommands.ArrayReferenceCommandSet.GetValuesCommand);
        packet.setNextValueAsArrayID(bigObjectArrayForOutOfMemoryID);
        packet.setNextValueAsInt(0); // first index
        packet.setNextValueAsInt(bigObjectArrayLength); // The number of components to retrieve
        try {
            commandStartTimeMlsec = System.currentTimeMillis();
        ...
        if ( ! printExpectedError(reply, JDWPConstants.Error.OUT_OF_MEMORY,
                                  "ArrayReference::GetValues command") ) {
            logWriter.println("\n");
            logWriter.println("==> Check of returned elements of bigObjectArrayForOutOfMemory...");
            int checkStatus = SUCCESS;
            byte arrayElementsTag = reply.getNextValueAsByte();
            int valuesNumber = reply.getNextValueAsInt();
            logWriter.println("==> Received array elements tag = " + arrayElementsTag
            ...
            if ( checkStatus != FAILURE ) {
                logWriter.println
                ("==> Check of returned elements of bigObjectArrayForOutOfMemory - OK.");
            } else {
                testCaseStatus = FAILURE;
            }
        }
        ...
        logWriter.println("==> testMemory005: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testMemory005: FAILED");
            return failed("testMemory005:");
        } else {
            logWriter.println("==> testMemory005: OK");
            return passed("testMemory005: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testMemory005: FAILED");
    return failed("==> testMemory005: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new MemoryTest005().test(args));
	}
}
