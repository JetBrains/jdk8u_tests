...
import org.apache.harmony.test.stress.classloader.NotSynchThreads.SupIntf.testManySupIntf_01;
        ...
        if ((testArgs.length - 3) <= 0) {
            ...
            if (l <= 2) {
                ...
                if (mode == 0) {
                    ...
                    loopTest = (Integer.valueOf(testThread.argsForTest[1]))
                            .intValue();
                    testThread.argsForTest[1] = Integer.toString(1);
                }
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < lmtLoop; i++) {
                ...
                for (int j = 0; j < lmtThrd; j++) {
                    if ((j % 2) == 0) {
                        tThread[j] = new testThread(j, true);
                    } else {
                        tThread[j] = new testThread(j, false);
                    }
                    tThread[j].start();
                }
...
class testThread extends Thread {
    ...
    public testThread(int index, boolean mode) {
        this.index = index;
        this.mSynch = mode;
    }
    ...
    public void run() {
        ...
        test01.cnt++;
        if (!mSynch) {
            testThread.argsForTest[1] = Integer
                    .toString(testManySupIntf1.loopTest);
            testManySupIntf1.tableResults[index] = t.test1(argsForTest);
        } else {
            try {
                for (int i = 0; i < testManySupIntf1.loopTest; i++) {
                    synchronized (test01.class) {
                        int ans = t.test1(argsForTest);
                        testManySupIntf1.tableResults[index] += ans;
                    }
                }
            } catch (Throwable e) {
                e.printStackTrace(System.out);
                testManySupIntf1.tableResults[index] += 105;
            }
        }
    }
}

class test01 extends testManySupIntf_01 {
