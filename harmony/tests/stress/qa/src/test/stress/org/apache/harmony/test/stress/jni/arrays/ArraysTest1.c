...
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest1_init(JNIEnv *env,
                                                              jclass c) {
...
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest1_doNativeCalc(
    JNIEnv *env,
    jobject thisObject,
    jint len,
    jint maxCnt,
  ...
  for (cnt = 0; cnt < maxCnt; cnt++) {
    ...
    if( (*env)->ExceptionCheck(env) ) {
      printf("Native code: Error writing array element, index=%d\n", cnt);
      free(tmp1);
      return 0;
    }
  }
