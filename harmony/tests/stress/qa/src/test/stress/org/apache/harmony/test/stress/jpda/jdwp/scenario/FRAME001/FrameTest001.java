 ...
 * of threads running in debuggee.
 */

public class FrameTest001 extends StressTestCase {

    public final static String DEBUGGEE_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.FRAME001.FrameDebuggee001";

    public final static String DEBUGGEE_SIGNATURE = "L"
            + DEBUGGEE_CLASS_NAME.replace('.', '/') + ";";

    public final static String THREAD_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.FRAME001.FrameDebuggee001_Thread";
     ...
     */
    public Result testFrame001() {

        logWriter.println("==> testFrame001: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = FrameDebuggee001.THREAD_NAME_PATTERN + i;
        ...
        for (int i = 0; i < startedThreadsNumber; i++) {
            ...
            if ( isOutOfMemory ) {
                logWriter.println("==> Command - ThreadReference.Frames");
                logWriter.println("==> Thread name = " + startedThreadsNames[i]);
                terminateDebuggee(SUCCESS, "MARKER_26.1");
                if ( testCaseStatus == FAILURE ) {
                    return failed("## FAILUREs were found out! ");
                } else {
                    return passed("==> OUT_OF_MEMORY - Expected result! ");
            ...
            if ( checkReplyForError(reply, JDWPConstants.Error.NONE,
                ...
                if ( i >= threadsNumberToResume ) {
                    ...
                    terminateDebuggee(FAILURE, "MARKER_26.2");
                    return failed("## FAILURE in ThreadReference.Frames command! ");
                }
                testCaseStatus = FAILURE;
                continue;
            ...
            if ( frames == 0 ) {
                if ( i >= threadsNumberToResume ) {
                    logWriter.println
                    ("## FAILURE: ThreadReference.Frames command returns unexpected number of frames = 0");
                    terminateDebuggee(FAILURE, "MARKER_26.3");
                    return failed("## FAILURE in ThreadReference.Frames command! ");
                }
                limitedPrintln
                ("## FAILURE: ThreadReference.Frames command returns unexpected number of frames = 0");
            ...
            if ( i < threadsNumberToResume ) {
                ...
                if ( framesPerThreadsNumber[i] != frames ) {
                    ...
                    terminateDebuggee(FAILURE, "MARKER_26.4");
                    return failed("## FAILURE in ThreadReference.Frames command! ");
                }
            }
            int foundStaticCheckedMethods = 0;
            ...
            for (int j = 0; j < frames; j++) {
                ...
                if ( i < threadsNumberToResume ) {
                    ...
                    if ( threadsFrameIDs[i][j] != frameID ) {
                        ...
                        terminateDebuggee(FAILURE, "MARKER_26.5");
                        return failed("## FAILURE in ThreadReference.Frames command! ");
                    }
                }
                Location location = reply.getNextValueAsLocation();
                ...
                if ( isOutOfMemory ) {
                    ...
                    logWriter.println("==> Thread name = " + startedThreadsNames[i]);
                    logWriter.println("==> Frames' number = " + j);
                    terminateDebuggee(SUCCESS, "MARKER_28.1");
                    if ( testCaseStatus == FAILURE ) {
                        return failed("## FAILUREs were found out! ");
                    } else {
                        return passed("==> OUT_OF_MEMORY - Expected result! ");
        ...
        logWriter.println("==> testFrame001: running time(mlsecs) = "
                + testRunTimeMlsec);
        if (testCaseStatus == FAILURE) {
            logWriter.println("==> testFrame001: FAILED");
            return failed("testFrame001:");
        } else {
            logWriter.println("==> testFrame001: OK");
            return passed("testFrame001: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testFrame001: FAILED");
    return failed("==> testFrame001: Unexpected Exception! ");
}
    }

    public static void main(String[] args) {
        System.exit(new FrameTest001().test(args));
    }
}