...
public class ExceptionsTest9 extends STExceptionsTest {
    ...
    public void doTest() {
        int depth = Integer.getInteger(
                "org.apache.harmony.test.stress.jni."
                        + "exceptions.ExceptionsTest9.depth", 100).intValue();

        try {
            nativeMethod(depth);
            testLog("Working thread",
                    "ExceptionsTest9 FAILED: Expected exception was not caught in Java code");
            ReliabilityRunner.debug("Test failed");
            ReliabilityRunner.mainTest.addError(this, new JNITestError());
        } catch (JNITestException e) {
            if (getCounter() == depth) {
                testLog("Working thread",
                        "ExceptionsTest9 PASSED: Expected exception caught.");
            } else {
                testLog("Working thread",
                        "ExceptionsTest9 FAILED: Invalid exceptions counter value: "
            ...
            testLog("Working thread",
                    "ExceptionsTest9: ERROR: Unexpected exception caught in Java code");
