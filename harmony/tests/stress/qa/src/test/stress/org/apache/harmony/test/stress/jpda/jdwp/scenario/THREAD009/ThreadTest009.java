 ...
 * in the same thread under memory stressing in debuggee.
 * The test expects that all commands result in expected returned value. 
 */
public class ThreadTest009 extends StressTestCase {

    int COMMANDS_TO_RUN = THREAD009_COMMANDS_TO_RUN;
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD009.ThreadDebuggee009";
     ...
     * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread009() {
        logWriter.println("==> testThread009: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee009.METHOD_TO_INVOKE_NAME);
        ...
        logWriter.println("\n");
        logWriter.println("==> Wait for debuggee to create memory stress...");
        ...
        String ObjectIDFieldName = "threadDebuggee009This";
        ...
        for (int i=0; i < COMMANDS_TO_RUN; i++ ) {
            ...
            while ( true ) {
                ...
                if ( methodToInvokeCallNumber == BAD_INT_VALUE ) {
                    ...
                    terminateDebuggee(FAILURE, "MARKER_04.2");
                    return failed
                    ("## FAILURE Can NOT get value for 'methodToInvokeCallNumber' variable from debuggee! ");
                }
        ...
        logWriter.println("==> testThread009: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread009: FAILED");
            return failed("testThread009:");
        } else {
            logWriter.println("==> testThread009: OK");
            return passed("testThread009: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread009: FAILED");
    return failed("==> testThread009: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest009().test(args));
	}
}
