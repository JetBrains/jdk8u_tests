...
public class ExceptionsTest2 extends STExceptionsTest {
    ...
    public void doTest() {
        int depth = Integer.getInteger(
                "org.apache.harmony.test.stress.jni."
                        + "exceptions.ExceptionsTest2.depth", 100).intValue();

        try {
            testLog("Working thread", "Calling a native method...");
            nativeMethod(depth);
            testLog("Working thread",
                    "ExceptionsTest2 FAILED: expected exception was not caught in Java code");
            ReliabilityRunner.debug("Test failed");
            ReliabilityRunner.mainTest.addError(this, new JNITestError());
        } catch (JNITestException e) {
            if (getCounter() == depth) {
                testLog("Working thread",
                        "ExceptionsTest2 PASSED: Expected exception caught.");
            } else {
                testLog("Working thread",
                        "ExceptionsTest2 FAILED: Invalid exceptions counter value: "
            ...
            testLog("Working thread",
                    "ExceptionsTest2: ERROR: Unexpected exception caught in Java code");
