 ...
 * Before each thread continues test method to cause expected 'BREAKPOINT'
 * and 'SINGLE_STEP' events the debuggee creates memory stress until
 * OutOfMemory happens.
 ...
 */
public class EventTest014 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT014.EventDebuggee014";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT014/EventDebuggee014_Thread;";
     ...
     * 'SINGLE_STEP' events are received.
     */
    public Result  testEvent014() {

        logWriter.println("==> testEvent014: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee014.THREAD_NAME_PATTERN + i;
        ...
        boolean[] isMethodEntryReceivedWithSucess = new boolean[startedThreadsNumber];
        measurableCodeStartTimeMlsec = System.currentTimeMillis();
        limitedPrintlnInit(20);
        for (int i=0; i < startedThreadsNumber; i++) {
            isMethodEntryReceivedWithSucess[i] = false;
            if ( methodEntryRequestIDs[i] != NO_REQUEST_ID ) {
                ...
                isMethodEntryReceivedWithSucess[i] = true;
            }
        }
        logWriter.println
        ...
        if ( receivingEventsStatus == FAILURE ) {
            ...
            terminateDebuggee(FAILURE, "MARKER_11.05");
        ...
        if ( clearingRequestsStatus == FAILURE ) {
            ...
            terminateDebuggee(FAILURE, "MARKER_11.06");
        ...
        limitedPrintlnInit(20);
        for (int i=0; i < startedThreadsNumber; i++) {
            if ( ! isMethodEntryReceivedWithSucess[i] ) {
                continue;   
            }
            ReplyPacket setEventReply = null;
        ...
        logWriter.println("\n");
        logWriter.println
        ("==> Wait for debuggee readiness to create memory stress ('SIGNAL_READY_02')...");
        measurableCodeStartTimeMlsec = System.currentTimeMillis();
        ...
        measurableCodeRunningTimeMlsec = System.currentTimeMillis() - measurableCodeStartTimeMlsec;
        logWriter.println
        ("==> Time(mlsecs) of waiting for debuggee readiness to create memory stress = " + 
                measurableCodeRunningTimeMlsec);
        

        logWriter.println("\n");
        logWriter.println
        ("==> Resume only debuggee main thread to create memory stress ...");
        resumeSignalThread("#3");
        
        logWriter.println("\n");
        logWriter.println
        ("==> Wait for debuggee to create memory stress ('SIGNAL_READY_03')...");
        measurableCodeStartTimeMlsec = System.currentTimeMillis();
        debuggeeSignal =  receiveThreadSignalWithWait(currentTimeout());
        ...
        measurableCodeRunningTimeMlsec = System.currentTimeMillis() - measurableCodeStartTimeMlsec;
        logWriter.println
        ("==> Time(mlsecs) of waiting for debuggee to create memory stress = " + 
                measurableCodeRunningTimeMlsec);
        
        logWriter.println
        ("==> Resume all debuggee to allow all threads to continue test method execution and cause " +
        ...
        if ( receivingEventsStatus == FAILURE ) {
            ...
            terminateDebuggee(FAILURE, "MARKER_17.1");
        ...
        if ( receivingEventsStatus == FAILURE ) {
            ...
            terminateDebuggee(FAILURE, "MARKER_17.2");
        ...
        logWriter.println("==> testEvent014: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent014: FAILED");
            return failed("testEvent014:");
        } else {
            logWriter.println("==> testEvent014: OK");
            return passed("testEvent014: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent014: FAILED");
    return failed("==> testEvent014: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest014().test(args));
	}
}
