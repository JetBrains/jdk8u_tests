 ...
 */
public class ThreadTest001 extends StressTestCase {

    final int COMMANDS_TO_RUN = THREAD001_COMMANDS_TO_RUN;
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD001.ThreadDebuggee001";
     ...
     * in the same thread. The test expects that all commands result in expected returned value. 
     */
    public Result  testThread001() {
        logWriter.println("==> testThread001: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee001.METHOD_TO_INVOKE_NAME);
        ...
        logWriter.println("==> testThread001: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread001: FAILED");
            return failed("testThread001:");
        } else {
            logWriter.println("==> testThread001: OK");
            return passed("testThread001: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread001: FAILED");
    return failed("==> testThread001: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest001().test(args));
	}
}
