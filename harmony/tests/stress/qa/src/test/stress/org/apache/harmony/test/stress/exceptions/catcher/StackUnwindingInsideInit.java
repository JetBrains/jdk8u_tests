...
public class StackUnwindingInsideInit extends Catcher {
    ...
    public void testUnwinding() {
        ...
        String propetyName = "org.apache.harmony.test.stress.exceptions."
                + "catcher.StackUnwindingInsideInit";
        deepUnwinding = Integer
                .getInteger(propetyName, 100).intValue();

        try {
            new ScrewAndThrowInsideInit(deepUnwinding);
    ...
    private class ScrewAndThrowInsideInit {
        ScrewAndThrowInsideInit(int deepUnwinding) throws Throwable {
            if (deepUnwinding > 0) {
                new ScrewAndThrowInsideInit(deepUnwinding - 1);
            } else {
                doThrow();
            }
        }
