 ...
 * saves result. Generates another array of Objects with very large length,
 * creates memory stressing by loading in memory very big array. Runs
 ...
 */
public class ObjectTest007 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.OBJECT007.ObjectDebuggee007";
    ...
    public long[] arrayForCompare1 = new long[StressDebuggee.OBJECT007_ARRAY_LENGTH];
    public long[] arrayForCompare2 = new long[StressDebuggee.OBJECT007_ARRAY_LENGTH];
     ...
     * <code>ObjectReference.ReferenceType</code> command and checks results.
     */
    public Result  testObject007() {
        logWriter.println("==> testObject007: START (" + new Date() + ")...");
...
try {
        ...
        logWriter.println("==> testObject007: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testObject007: FAILED");
            return failed("testObject007:");
        } else {
            logWriter.println("==> testObject007: OK");
            return passed("testObject007: OK");
        }
    ...
    logWriter.println("==> testObject007: FAILED");
    return failed("==> testObject007: Unexpected Exception! ");
}
    }
    ...
    public int checkArray(String arrayName, long[] arrayForCompare) {
        ...
        packet.setNextValueAsInt(StressDebuggee.OBJECT007_ARRAY_LENGTH);
    ...
    public static void main(String[] args) {
        System.exit(new ObjectTest007().test(args));
    }
}
