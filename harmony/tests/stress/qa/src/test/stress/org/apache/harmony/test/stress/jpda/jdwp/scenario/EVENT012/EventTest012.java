 ...
 * events are received.
 */
public class EventTest012 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT012.EventDebuggee012";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT012/EventDebuggee012_Thread;";
     ...
     * events are received.
     */
    public Result  testEvent012() {

        logWriter.println("==> testEvent012: START (" + new Date() + ")...");
...
try {
        ...
        logWriter.println
        ("==> Time(mlsecs) of waiting for for debuggee to start big number of treads = " + 
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee012.THREAD_NAME_PATTERN + i;
        ...
        if ( receivingEventsStatus == FAILURE ) {
            ...
            terminateDebuggee(FAILURE, "MARKER_12.1");
        ...
        logWriter.println("==> testEvent012: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent012: FAILED");
            return failed("testEvent012:");
        } else {
            logWriter.println("==> testEvent012: OK");
            return passed("testEvent012: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent012: FAILED");
    return failed("==> testEvent012: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest012().test(args));
	}
}
