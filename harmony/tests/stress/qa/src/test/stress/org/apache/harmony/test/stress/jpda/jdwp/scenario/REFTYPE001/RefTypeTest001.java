 ...
 *  
 */
public class RefTypeTest001 extends StressTestCase {

    public final static String DEBUGGEE_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.REFTYPE001.RefTypeDebuggee001";
     ...
     *  
     */
    public Result testRefType001() {
        int testCaseStatus = SUCCESS;
        logWriter.println("==> testRefType001: START (" + new Date() + ")...");
...
try {
        ...
        if ( saveRefTypeIDsStatus ) {
            ...
            int allClasses = refTypesArray.getLength();
            if (!checkClasses(refTypesArray, signaturesArray, tagsArray, allClasses)) {
                logWriter.println("#FAILURE: during checking classes");
                testCaseStatus = FAILURE;
            } else {
                logWriter.println("==> Checking classes PASSED");
            }
            
            if (!checkClass("Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/REFTYPE001/RefType001_TestClass01;", "RefTypeDebuggee001.java", refTypesArray)) {
                logWriter.println("#FAILURE: during check class RefType001_TestClass01");
                testCaseStatus = FAILURE;
            }
            
            if (!checkClass("Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/REFTYPE001/RefType001_TestClass02;", "RefTypeDebuggee001.java", refTypesArray)) {
                logWriter.println("#FAILURE: during check class RefType001_TestClass01");
                testCaseStatus = FAILURE;
            }
        }
        ...
        logWriter.println("==> testRefType001: running time(mlsecs) = "
                + testRunTimeMlsec);
        if (testCaseStatus == FAILURE) {
            logWriter.println("==> testRefType001: FAILED");
            return failed("testRefType001:");
        } else {
            logWriter.println("==> testRefType001: OK");
            return passed("testRefType001: OK");
        }
    ...
    logWriter.println("==> testRefType001: FAILED");
    return failed("==> testRefType001: Unexpected Exception! ");
}
    }

    public static void main(String[] args) {
        System.exit(new RefTypeTest001().test(args));
    }
}