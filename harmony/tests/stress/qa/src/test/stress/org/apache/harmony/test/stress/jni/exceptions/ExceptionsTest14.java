...
public class ExceptionsTest14 extends MTExceptionsTest {

    private static native void nativeMethod(int cnt, int id)
            throws JNITestException;

    protected native void init(int threadsCnt);

    public ExceptionsTest14(int id) {
        super(id);
    }

    public ExceptionsTest14() {
        super(-1);
    }
    ...
    public void doTest() {
        int depth = Integer.getInteger(
                "org.apache.harmony.test.stress.jni."
                        + "exceptions.ExceptionsTest14.depth", 100).intValue();
        try {
            nativeMethod(depth, threadId);
            testLog("Working thread",
                    "ExceptionsTest14 FAILED: No exception caught in Java code");
            ...
            if (getCounter(threadId) != depth) {
                testLog("Working thread",
                        "ExceptionsTest14 FAILED: Invalid exceptions counter value: "
            ...
            testLog("Working thread",
                    "ExceptionsTest14: ERROR: Unexpected exception caught in Java code:");
