...
package org.apache.harmony.test.stress.classloader.SynchThreads.ConstantPool;

import junit.framework.TestCase;

import org.apache.harmony.test.stress.classloader.share.ClassLoaderTestError;
import org.apache.harmony.test.share.stress.ReliabilityRunner;
import org.apache.harmony.test.stress.classloader.NotSynchThreads.ConstantPool.testCPLmt_0;
...
public class testCPLmt extends TestCase {
    ...
    public void test() {
        ...
        ReliabilityRunner.debug("Start testCPLmt");
        ...
        try {
            ...
            for (int i = 0; i < lmtLoop; i++) {
                ...
                for (int j = 0; j < lmtThrd; j++) {
                    if (tableResults[j] != (104 * loopTest)) {
...
class testThread extends Thread {
    ...
    public void run() {
        ...
        test00.cnt++;
        try {
            for (int i = 0; i < testCPLmt.loopTest; i++) {
                synchronized (test00.class) {
                    int ans = t.test1(argsForTest);
                    testCPLmt.tableResults[index] += ans;
                }
            }
        } catch (Throwable e) {
            e.printStackTrace(System.out);
            testCPLmt.tableResults[index] += 105;
        }
    }
}

class test00 extends testCPLmt_0 {
