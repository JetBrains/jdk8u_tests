...
public class StringsTest11 extends MTTest {

    public StringsTest11(int threadId) {
        super(threadId);
    }

    public StringsTest11() {
        super(-1);
    }
        ...
        try {
            int length = Integer.getInteger(
                    "org.apache.harmony.test.stress.jni."
                            + "strings.StringsTest11.length", 1000).intValue();
            int maxStrCount = Integer.getInteger(
                    "org.apache.harmony.test.stress.jni."
                            + "strings.StringsTest11.maxStrCount", 1000)
                    .intValue();
            ...
            if (res == 0) {
                testLog("Working thread",
                        "StringsTest11 FAILED: native method returned 0");
                ReliabilityRunner.debug("Test failed");
                ReliabilityRunner.mainTest.addError(this, new JNITestError());
            }
        } catch (Throwable t) {
            testLog("Working thread", "StringsTest11: ERROR: Exception caught:");
