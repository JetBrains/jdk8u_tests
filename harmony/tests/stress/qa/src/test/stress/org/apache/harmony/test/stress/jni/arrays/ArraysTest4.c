...
#include<jni.h>
#include<stdlib.h>
#include"ArraysTest4.h"
#include"share.h"

JNIEXPORT void JNICALL
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest4_init(JNIEnv *env,
                                                              jclass c) {
...
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest4_doNativeCalc(
    JNIEnv *env,
    jobject thisObject,
    jint len,    
    jint maxCnt,
  ...
  printf("Performing calculations on %d arrays...\n", cnt);
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (!comp) {
      printf("FAILED (compare result is FALSE), i=%d\n", i);
      free(tmp1);
      return 0;
    }
  ...
  printf("Done calculations on %d arrays\n", cnt);
  free(tmp1);
  return 1;
}
