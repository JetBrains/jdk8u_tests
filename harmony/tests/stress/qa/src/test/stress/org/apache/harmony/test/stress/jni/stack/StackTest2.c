...
Java_org_apache_harmony_test_stress_jni_stack_StackTest2_init(JNIEnv *env,
                                                              jclass c,
                                                              jint cnt) {
...
Java_org_apache_harmony_test_stress_jni_stack_StackTest2_nativeMethod(JNIEnv* env,
                                                                      jclass c,
                                                                      jintArray intArray,
                                                                      jint len,
                                                                      jint cnt){
  ...
  if (res == NULL) {    
    printf("Native code: NewIntArray() failed at cnt=%d\n", (int)cnt);
    StackTest_free_arrays(env, arr, calc_res, NULL);
    return NULL;
  }
  ...
  if (cnt < maxCnt) {
    ...
    if ((*env)->ExceptionCheck(env)) {
      printf("Native code: exception caught in native method\n");
      StackTest_free_arrays(env, arr, calc_res, NULL);
      return NULL;
    }
    if (res2 == NULL) {
