...
public class StringsTest8 extends MTTest {

    public StringsTest8(int threadId) {
        super(threadId);
    }
        ...
        try {
            int length = Integer.getInteger(
                    "org.apache.harmony.test.stress.jni."
                            + "strings.StringsTest8.length", 100000).intValue();
            int maxStrCount = Integer.getInteger(
                    "org.apache.harmony.test.stress.jni."
                            + "strings.StringsTest8.maxStrCount", 10)
                    .intValue();
            ...
            if (res == 0) {
                testLog("Working thread",
                        "StringsTest8 FAILED: native method returned 0");
                ReliabilityRunner.debug("Test failed");
                ReliabilityRunner.mainTest.addError(this, new JNITestError());
            }
        } catch (Throwable t) {
            testLog("Working thread", "StringsTest8: ERROR: Exception caught:");
