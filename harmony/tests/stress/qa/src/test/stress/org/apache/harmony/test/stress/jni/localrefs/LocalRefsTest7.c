...
#include<stdio.h>
#include<jni.h>
#include"LocalRefsTest7.h"
#include"share.h"

JNIEXPORT void JNICALL
Java_org_apache_harmony_test_stress_jni_localrefs_LocalRefsTest7_init(JNIEnv* env,
                                                                      jclass clazz) {
  LocalRefsTest_init(env, clazz);
}

JNIEXPORT jint JNICALL
Java_org_apache_harmony_test_stress_jni_localrefs_LocalRefsTest7_nativeMethod(JNIEnv* env,
                                                                              jobject thisObject,
                                                                              jint cnt,
                                                                              jint len) {
  int i;
  ...
  if (mid == NULL) {
    printf("Native code: Error getting method ID for getObjectArray\n");
    return -1;
  }
