 ...
 */
public class ThreadTest008 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD008.ThreadDebuggee008";
    ...
    /**
      ...
      * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread008() {
        logWriter.println("==> testThread008: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee008.METHOD_TO_INVOKE_NAME);
        ...
        logWriter.println("\n");
        logWriter.println
        ("==> Send ObjectReference.InvokeMethod asynchronous commands (" + commandsToRun +
                ") without waiting for reply...");
        String ObjectIDFieldName = "threadDebuggee008This";
        ...
        logWriter.println("==> testThread008: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread008: FAILED");
            return failed("testThread008:");
        } else {
            logWriter.println("==> testThread008: OK");
            return passed("testThread008: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread008: FAILED");
    return failed("==> testThread008: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest008().test(args));
	}
}
