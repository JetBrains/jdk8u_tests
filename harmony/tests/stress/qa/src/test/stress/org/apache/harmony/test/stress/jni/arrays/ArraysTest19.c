...
#include<jni.h>
#include<stdlib.h>
#include"ArraysTest19.h"
#include"share.h"

JNIEXPORT void JNICALL
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest19_init(JNIEnv *env,
                                                                 jclass c) {
...
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest19_doNativeCalc(
    JNIEnv *env,
    jobject thisObject,
    jint len,
    jint maxCnt,
  ...
  for (cnt = 0; cnt < maxCnt; cnt++) {
    ...
    if ( (*env)->MonitorEnter(env, clazz) < 0) {
      printf("Native code: Cannot set monitor\n");
      return 0;
    }
