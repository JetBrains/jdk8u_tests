 ...
 */
public class EventTest006 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT006.EventDebuggee006";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT006/EventDebuggee006_Thread;";
     ...
     * 'METHOD_EXIT' events are received.
     */
    public Result  testEvent006() {

        logWriter.println("==> testEvent006: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee006.THREAD_NAME_PATTERN + i;
        ...
        logWriter.println("==> testEvent006: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent006: FAILED");
            return failed("testEvent006:");
        } else {
            logWriter.println("==> testEvent006: OK");
            return passed("testEvent006: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent006: FAILED");
    return failed("==> testEvent006: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest006().test(args));
	}
}
