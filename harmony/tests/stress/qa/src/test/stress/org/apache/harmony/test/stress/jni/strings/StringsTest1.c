...
#include<jni.h>
#include<stdlib.h>
#include"StringsTest1.h"
#include"share.h"

JNIEXPORT void JNICALL
Java_org_apache_harmony_test_stress_jni_strings_StringsTest1_init(JNIEnv* env,
                                                                  jclass c) {
  StringsTest_init(env, c);
}

JNIEXPORT jint JNICALL
Java_org_apache_harmony_test_stress_jni_strings_StringsTest1_nativeMethod(
    JNIEnv *env,
    jobject thisObject,
    jint len,
    jint maxCnt) {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if(!StringsTest_do_compare(calc_res, tmp, len)) {
      ...
      printf("Native code: Strings comparison failed\n");
      StringsTest_free_2d_array(env, arrays, cnt);
      StringsTest_free_arrays(env, calc_res, NULL);
      return 0;
    }
