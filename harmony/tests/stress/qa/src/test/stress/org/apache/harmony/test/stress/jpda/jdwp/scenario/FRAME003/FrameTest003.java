...
public class FrameTest003 extends StressTestCase {

    public final static String DEBUGGEE_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.FRAME003.FrameDebuggee003";

    public final static String DEBUGGEE_SIGNATURE = "L"
            + DEBUGGEE_CLASS_NAME.replace('.', '/') + ";";

    public final static String THREAD_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.FRAME003.FrameDebuggee003_Thread";
     ...
     */
    public Result testFrame003() {

        logWriter.println("==> testFrame003: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = FrameDebuggee003.THREAD_NAME_PATTERN + i;
        ...
        logWriter.println("==> testFrame003: running time(mlsecs) = "
                + testRunTimeMlsec);
        if (testCaseStatus == FAILURE) {
            logWriter.println("==> testFrame003: FAILED");
            return failed("testFrame003:");
        } else {
            logWriter.println("==> testFrame003: OK");
            return passed("testFrame003: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testFrame003: FAILED");
    return failed("==> testFrame003: Unexpected Exception! ");
}
    }

    public static void main(String[] args) {
        System.exit(new FrameTest003().test(args));
    }
}