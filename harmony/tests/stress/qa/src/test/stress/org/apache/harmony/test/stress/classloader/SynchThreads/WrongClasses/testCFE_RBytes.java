...
public class testCFE_RBytes extends TestCase {
            ...
            for (int i = 0; i < lmtLoop; i++) {
                ...
                for (int j = 0; j < lmtThrd; j++) {
                    if (tableResults[j] != (ReliabilityRunner.RESULT_PASS * loopTest)) {
                        ReliabilityRunner
                                .debug("Incorrect answer. Failed thread: "
                                        + j
                                        + "  "
                                        + tableResults[j]
                                        + " must be "
    ...
    static class testThread extends Thread {
        ...
        public void run() {
            testCFE_RBytes_00 t = new testCFE_RBytes_00();
            testCFE_RBytes_00.cnt++;
            try {
                for (int i = 0; i < testCFE_RBytes.loopTest; i++) {
                    synchronized (testCFE_RBytes_00.class) {
                        int ans = t.test(argsForTest);
                        testCFE_RBytes.tableResults[index] += ans;
                    }
                }
            } catch (Throwable e) {
                e.printStackTrace(System.out);
                testCFE_RBytes.tableResults[index] += 105;
            }
        }
    }
}
