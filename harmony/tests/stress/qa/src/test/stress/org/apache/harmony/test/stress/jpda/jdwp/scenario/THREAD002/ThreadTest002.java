 ...
 */
public class ThreadTest002 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD002.ThreadDebuggee002";
    ...
    /**
      ...
      * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread002() {
        logWriter.println("==> testThread002: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee002.METHOD_TO_INVOKE_NAME);
        ...
        for (int i=0; i < sentCommands; i++ ) {
            ...
            if ( reply == null ) {
                ...
                ("## FAILURE: Can NOT receive reply for ClassType.InvokeMethod command: " +
                        "Command index = " + i);
                terminateDebuggee(FAILURE, "MARKER_08.0.1`");
                return failed("## FAILURE while receiving reply for ClassType.InvokeMethod command");
            }
            receivedReplies++;
        ...
        if ( outOfMemory ) {
            if ( testCaseStatus == FAILURE ) {
                ...
                terminateDebuggee(SUCCESS, "MARKER_8.2");
                return passed
        ...
        logWriter.println("==> testThread002: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread002: FAILED");
            return failed("testThread002: ");
        } else {
            logWriter.println("==> testThread002: OK");
            return passed("testThread002: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread002: FAILED");
    return failed("==> testThread002: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest002().test(args));
	}
}
