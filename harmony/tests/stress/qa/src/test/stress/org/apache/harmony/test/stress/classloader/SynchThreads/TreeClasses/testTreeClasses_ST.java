 ...
 */
package org.apache.harmony.test.stress.classloader.SynchThreads.TreeClasses;

import junit.framework.TestCase;

import org.apache.harmony.test.stress.classloader.share.ClassLoaderTestError;
import org.apache.harmony.test.share.stress.ReliabilityRunner;
import org.apache.harmony.test.stress.classloader.OneThread.TreeClasses.testTreeClasses;

public class testTreeClasses_ST extends TestCase {
    ...
    public void test() {
        ...
        for (testArgsCount = 0; testArgsCount < ARGS_COUNT; testArgsCount++) {
            preTestArgs[testArgsCount] = System
                    .getProperty("org.apache.harmony.test."
                            ...
                            + "TreeClasses.testTreeClasses_ST.arg"
                            + testArgsCount);
        ...
        // testArgs[4] - correct classes package name
        // testArgs[5] - number of used classes
            ...
            if (l <= 2) {
                ...
                if (mode == 0) {
                    ...
                    loopTest = (Integer.valueOf(testThread.argsForTest[0]))
                            .intValue();
                    testThread.argsForTest[0] = Integer.toString(1);
                }
            }
        }

        ReliabilityRunner.debug("Start testTreeClasses_ST");
        ReliabilityRunner.debug("Number of iterations: " + lmtLoop);
        ReliabilityRunner.debug("Number of created thread: " + lmtThrd);

        testTreeClasses.logIn = false;
        testTreeClasses.checkIn = false;
    ...
    static class testThread extends Thread {
        ...
        public void run() {
            ...
            test00.cnt++;
            try {
                for (int i = 0; i < testTreeClasses_ST.loopTest; i++) {
                    synchronized (test00.class) {
                        int ans = t.test1(argsForTest);
                        testTreeClasses_ST.tableResults[index] += ans;
                    }
                }
            } catch (Throwable e) {
                e.printStackTrace(System.out);
                testTreeClasses_ST.tableResults[index] += 105;
            }
        }
    }
...
class test00 extends testTreeClasses {
