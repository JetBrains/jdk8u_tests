 ...
 * and disposes all ThreadIDs. For each thread test checks
 * ReferenceType, Name, and Status, and expects proper number of
 * INVALID_OBJECTID errors.
 */
public class ObjectTest003 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.OBJECT003.ObjectDebuggee003";
    ...
    public final static int TEST_NUMBER = 3; 
     ...
     * INVALID_OBJECTID errors.
     */
    public Result  testObject003() {
        logWriter.println("==> testObject003: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = ObjectDebuggee003.THREAD_NAME_PATTERN + i;
            ...
            for (int i = 0; i < allThreadsNumber; i++ ) {
                ...
                for (int j = 0; j < startedThreadsNumber; j++ ) {
                    if ( startedThreadsNames[j].equals(threadName) ) {
                        ...
                        if (!source.equals("ObjectDebuggee003.java")) {
                            logWriter.println("##FAILURE: received source of thread class = "+source+" instead of ObjectDebuggee003.java");
                            testCaseStatus = FAILURE;
                        }
                        
                        String signature = debuggeeWrapper.vmMirror.getClassSignature(refType);
                        if (!signature.equals("Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/OBJECT003/ObjectDebuggee003_Thread;")) {
                            logWriter.println("##FAILURE: received signature of thread class = "+signature+" instead of Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/OBJECT003/ObjectDebuggee003_Thread;");
                            testCaseStatus = FAILURE;
                        }
        ...
        int objectsToDispose = allThreadIDs.length;
        int result = disposeObj(objectsToDispose, allThreadIDs);
        if (result == -1) {
            logWriter.println("## FAILURE: disposing objects - FAILED");
            testCaseStatus = FAILURE;
        } else {
            logWriter.println("==> Disposing objects - PASSED");
            result = checkThreads(allThreadIDs, startedThreadsNames, TEST_NUMBER, objectsToDispose);
            if (result != 1) {
        ...
        logWriter.println("==> testObject003: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testObject003: FAILED");
            return failed("testObject003:");
        } else {
            logWriter.println("==> testObject003: OK");
            return passed("testObject003: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testObject003: FAILED");
    return failed("==> testObject003: Unexpected Exception! ");
}
    }
       

    public static void main(String[] args) {
        System.exit(new ObjectTest003().test(args));
    }
}
