...
import org.apache.harmony.test.stress.classloader.share.CorrectClasses.testCorrectClass;

public class testCorrectClass_NST extends TestCase {
        ...
        for (testArgsCount = 0; testArgsCount < ARGS_COUNT; testArgsCount++) {
            preTestArgs[testArgsCount] = System
                    .getProperty("org.apache.harmony.test."
                            ...
                            + "ManyClasses.testCorrectClass_NST.arg"
                            + testArgsCount);
            ...
            for (int i = 0; i < lmtLoop; i++) {
                ...
                for (int j = 0; j < lmtThrd; j++) {
                    if (tableResults[j] != ReliabilityRunner.RESULT_PASS) {
                        ReliabilityRunner
                                .debug("Incorrect resulted value. Failed thread:  "
                                        + j
                                        + "  value:"
                                        + tableResults[j]
                                        + "   iteration: " + i);
    ...
    static class testThread extends Thread {
        ...
        public void run() {
            testCorrectClass_NST.tableResults[index] = new testCorrectClass()
                    .test(argsForTest);
