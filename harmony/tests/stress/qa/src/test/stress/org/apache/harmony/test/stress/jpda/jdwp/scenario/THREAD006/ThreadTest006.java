 ...
 */
public class ThreadTest006 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD006.ThreadDebuggee006";
    ...
    /**
      ...
      * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread006() {
        logWriter.println("==> testThread006: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee006.METHOD_TO_INVOKE_NAME);
        ...
        logWriter.println("==> testThread006: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread006: FAILED");
            return failed("testThread006:");
        } else {
            logWriter.println("==> testThread006: OK");
            return passed("testThread006: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread006: FAILED");
    return failed("==> testThread006: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest006().test(args));
	}
}
