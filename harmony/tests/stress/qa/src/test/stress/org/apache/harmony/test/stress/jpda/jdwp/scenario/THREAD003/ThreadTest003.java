 ...
 */
public class ThreadTest003 extends StressTestCase {
    
    final int COMMANDS_TO_RUN = THREAD003_COMMANDS_TO_RUN;
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD003.ThreadDebuggee003";
     ...
     * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread003() {
        logWriter.println("==> testThread003: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee003.METHOD_TO_INVOKE_NAME);
        ...
        logWriter.println("==> testThread003: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread003: FAILED");
            return failed("testThread003:");
        } else {
            logWriter.println("==> testThread003: OK");
            return passed("testThread003: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread003: FAILED");
    return failed("==> testThread003: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest003().test(args));
	}
}
