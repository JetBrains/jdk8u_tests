 ...
 */
public class ThreadTest010 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.THREAD010.ThreadDebuggee010";
    ...
    /**
      ...
      * The test expects that all commands result in expected returned value. 
     */
    public Result  testThread010() {
        logWriter.println("==> testThread010: START (" + new Date() + ")...");
...
try {
        ...
        long toInvokeMethodID = debuggeeWrapper.vmMirror.getMethodID
            (debuggeeRefTypeID, ThreadDebuggee010.METHOD_TO_INVOKE_NAME);
        ...
        String ObjectIDFieldName = "threadDebuggee010This";
        ...
        logWriter.println("==> testThread010: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testThread010: FAILED");
            return failed("testThread010:");
        } else {
            logWriter.println("==> testThread010: OK");
            return passed("testThread010: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testThread010: FAILED");
    return failed("==> testThread010: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new ThreadTest010().test(args));
	}
}
