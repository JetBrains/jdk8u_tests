...
public class FrameTest004 extends StressTestCase {

    public final static String DEBUGGEE_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.FRAME004.FrameDebuggee004";

    public final static String DEBUGGEE_SIGNATURE = "L"
            + DEBUGGEE_CLASS_NAME.replace('.', '/') + ";";

    public final static String THREAD_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.FRAME004.FrameDebuggee004_Thread";
     ...
     */
    public Result testFrame004() {

        logWriter.println("==> testFrame004: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = FrameDebuggee004.THREAD_NAME_PATTERN + i;
        ...
        for (int i = 0; i < startedThreadsNumber; i++) {
            ...
            if ( isOutOfMemory ) {
                logWriter.println("==> Command - ThreadReference.Frames");
                logWriter.println("==> Thread name = " + startedThreadsNames[i]);
                terminateDebuggee(SUCCESS, "MARKER_16.1");
                if ( testCaseStatus == FAILURE ) {
            ...
            for (int j = 0; j < frames; j++) {
                ...
                if ( methodName == OUT_OF_MEMORY ) {
                    logWriter.println("==> OutOfMemory while getting method name for Thread name = " +
                            startedThreadsNames[i]);
                    terminateDebuggee(SUCCESS, "MARKER_17");
                    if ( testCaseStatus == FAILURE ) {
                        return failed
                ...
                if ( isOutOfMemory ) {
                    ...
                    logWriter.println("==> Thread name = " + startedThreadsNames[i]);
                    logWriter.println("==> Frames' number = " + j);
                    terminateDebuggee(SUCCESS, "MARKER_18.1");
                    if ( testCaseStatus == FAILURE ) {
                        return failed("## FAILUREs were found out! ");
                    } else {
                        return passed("==> OUT_OF_MEMORY - Expected result! ");
        ...
        for (int i=0; i < threadsNumberToResume; i++) {
            ...
            if ( printExpectedError(reply, JDWPConstants.Error.OUT_OF_MEMORY,
                ...
                logWriter.println("==> Thread name = " +
                        startedThreadsNames[i]);
                terminateDebuggee(SUCCESS, "MARKER_21");
                return passed
        ...
        for (int i=0; i < threadsNumberToResume; i++) {
            ...
            if ( isOutOfMemory ) {
                logWriter.println("==> Command - ThreadReference.Frames");
                logWriter.println("==> Thread name = " + startedThreadsNames[i]);
                terminateDebuggee(SUCCESS, "MARKER_22.1");
            ...
            if ( isOutOfMemory ) {
                ...
                logWriter.println("==> Thread name = " + startedThreadsNames[i]);
                logWriter.println("==> Frames' number = " + 0);
                terminateDebuggee(SUCCESS, "MARKER_24.1");
        ...
        logWriter.println("==> testFrame004: running time(mlsecs) = "
                + testRunTimeMlsec);
        if (testCaseStatus == FAILURE) {
            logWriter.println("==> testFrame004: FAILED");
            return failed("testFrame004:");
        } else {
            logWriter.println("==> testFrame004: OK");
            return passed("testFrame004: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testFrame004: FAILED");
    return failed("==> testFrame004: Unexpected Exception! ");
}
    }

    public static void main(String[] args) {
        System.exit(new FrameTest004().test(args));
    }
}