 ...
 * returned RefTypeIDs. Then, test loads some number of new classes, sends
 * <code>VirtualMachine.ClassesBySignature</code> and checks that RefTypeIDs of new classes are unique.
 * Then checks if all RefTypeIDs have expected SourceFile and signatures.
 *  
 */
public class RefTypeTest003 extends StressTestCase {

    public final static String DEBUGGEE_CLASS_NAME = "org.apache.harmony.test.stress.jpda.jdwp.scenario.REFTYPE003.RefTypeDebuggee003";
     ...
     *  
     */
    public Result testRefType003() {
        int testCaseStatus = SUCCESS;
        logWriter.println("==> testRefType003: START (" + new Date() + ")...");
...
try {
        ...
        logWriter.println("REFTYPE.LENGTH = " + refTypesArray.getLength());
        
        logWriter.println("receiving 'SIGNAL_READY_03' Signal from debuggee...");
        logWriter.println("==> Wait for loading by debuggee some classes..");
        resumeDebuggee("#5");
        debuggeeSignal = receiveSignalWithWait(currentTimeout());
        ...
        if (!checkNewClasses(refTypesArray, signaturesArray, tagsArray, 3)) {
        ...
        logWriter.println("==> Wait for loading by debuggee some another classes..");
        debuggeeSignal = receiveSignalWithWait(currentTimeout());
        ...
        logWriter.println("==> Check this another classes..");
        for (int i = 1; i < 4; i++) {
            if (!checkClass("Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/REFTYPE003/NewTestClass"+i+";", "RefTypeDebuggee003.java", refTypesArray)) {
                testCaseStatus = FAILURE;
                logWriter.println("##FAILURE: during checking class: NewTestClass"+i);
            }
        }
        ...
        logWriter.println("==> testRefType003: running time(mlsecs) = "
                + testRunTimeMlsec);
        if (testCaseStatus == FAILURE) {
            logWriter.println("==> testRefType003: FAILED");
            return failed("testRefType003:");
        } else {
            logWriter.println("==> testRefType003: OK");
            return passed("testRefType003: OK");
        }
    ...
    logWriter.println("==> testRefType003: FAILED");
    return failed("==> testRefType003: Unexpected Exception! ");
}
    }

    public static void main(String[] args) {
        System.exit(new RefTypeTest003().test(args));
    }
}