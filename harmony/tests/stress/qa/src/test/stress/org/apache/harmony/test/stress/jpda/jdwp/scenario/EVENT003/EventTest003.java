 ...
 */
public class EventTest003 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT003.EventDebuggee003";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT003/EventDebuggee003_Thread;";
     ...
     * and all expected 'METHOD_EXIT' events are received.
     */
    public Result  testEvent003() {

        logWriter.println("==> testEvent003: START (" + new Date() + ")...");
...
try {
        ...
        String debuggeeSignal =  receiveSignalWithWait(
                currentTimeout(testStartTimeMlsec, testTimeout));
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee003.THREAD_NAME_PATTERN + i;
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            while ( methodEntryRequestID != NO_REQUEST_ID ) {
                ParsedEvent event = receiveEvent(NO_REQUEST_ID, JDWPConstants.EventKind.METHOD_ENTRY, 
                        startedThreadsIDs[i], currentTimeout(testStartTimeMlsec, testTimeout));
                if ( event == null ) {
        ...
        ParsedEvent event = receiveEvent(
                NO_REQUEST_ID, NO_EVENT_KIND, ANY_THREAD,
                currentTimeout(testStartTimeMlsec, testTimeout));
        byte eventKind = event.getEventKind();
        ...
        logWriter.println("==> testEvent003: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent003: FAILED");
            return failed("testEvent003:");
        } else {
            logWriter.println("==> testEvent003: OK");
            return passed("testEvent003: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent003: FAILED");
    return failed("==> testEvent003: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest003().test(args));
	}
}
