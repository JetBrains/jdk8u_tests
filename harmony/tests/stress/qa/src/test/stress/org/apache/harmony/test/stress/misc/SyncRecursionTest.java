...
public class SyncRecursionTest extends TestCase {
    ...
    public void test() {
        try {
            ...
            if (count > 255) {
                ReliabilityRunner.debug("SyncRecursionTest PASSED, count=" + count);
            } else {
                ReliabilityRunner.debug("SyncRecursionTest FAILED: too short stack ("
                                + count + ")");
                ReliabilityRunner.debug("Test failed");
                ReliabilityRunner.mainTest.addError(this, new MiscTestError());
            }
    ...
    /**
     * A synchronized method which calls itself until SOF occurs
     */
