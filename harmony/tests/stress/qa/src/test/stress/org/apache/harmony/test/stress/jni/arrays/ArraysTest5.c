...
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest5_init(JNIEnv *env,
                                                              jclass c) {
 ...
 */
JNIEXPORT void JNICALL
Java_org_apache_harmony_test_stress_jni_arrays_ArraysTest5_doNativeCalc(
        JNIEnv *env,
        jobject thisObject,
        jint len,
        jint maxCnt,
  ...
  for (i = 0; i < cnt; i++) {
    jintArray arr;                          // Current array to be calculated
    jint* intArr1;                          // C representation of arr

    arr = (*env)->GetObjectArrayElement(env, arrays, i);
    if ( (*env)->ExceptionCheck(env) ) {
      printf("Native code: Error getting array element of type int[], index=%d\n",
            i);
      free(tmp1);
      return;
    }
