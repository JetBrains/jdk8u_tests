...
#include<jni.h>
#include<stdlib.h>
#include"StringsTest4.h"
#include"share.h"

JNIEXPORT void JNICALL
Java_org_apache_harmony_test_stress_jni_strings_StringsTest4_init(JNIEnv* env,
                                                                jclass c) {
  StringsTest_init(env, c);
}

JNIEXPORT jint JNICALL
Java_org_apache_harmony_test_stress_jni_strings_StringsTest4_nativeMethod(
    JNIEnv *env,
    jobject thisObject,
    jint len,
    jint maxCnt) {
  ...
  for (i = 0; i < cnt; i++) {
    const jchar* tmp;
    jstring string;                 // Current string to process
    jstring res;                    // Calculation result string
    int critical = 1;               // Flag to determine if GetStringCritical
                                    // works as expected
