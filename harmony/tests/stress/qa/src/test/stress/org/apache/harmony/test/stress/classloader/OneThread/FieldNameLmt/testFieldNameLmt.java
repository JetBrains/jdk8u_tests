...
package org.apache.harmony.test.stress.classloader.OneThread.FieldNameLmt;
...
public class testFieldNameLmt extends TestCase {
    ...
    public void test() {
        ...
        for (testArgsCount = 0; testArgsCount < ARGS_COUNT; testArgsCount++) {
            preTestArgs[testArgsCount] = System
                    .getProperty("org.apache.harmony.test."
                            ...
                            + "FieldNameLmt.testFieldNameLmt.arg"
                            + testArgsCount);
        ...
        pkgN = testArgs.length <= 0 ? this.getClass().getPackage().getName()
                : testArgs[0];
        cntLoop = testArgs.length <= 1 ? CNT_LOOP : Integer
                .valueOf(testArgs[1]).intValue();
        cntClss = testArgs.length <= 2 ? CNT_C : Integer.valueOf(testArgs[2])
                .intValue();
        if (cntClss > CNT_C) {
            cntClss = CNT_C;
        }

        // cntFlds = testArgs.length <= 3 ? CNT_F :
        // Integer.valueOf(testArgs[3]).intValue();
        // if (cntFlds > CNT_F) {
        // cntFlds = CNT_F;
        // }
        cntFlds = CNT_F;
        // int notPass = 0;
        ReliabilityRunner.debug("Loading classes which declare fields with long names");
        ...
        for (int j = 0; j < cntLoop; j++) {
            ...
            for (int i = 0; i < clss.length; i++) {
                try {
                    ...
                    if (flds[i].length < cntFlds) {
                        ReliabilityRunner
                                .debug("Failed. Wrong fields number in class:"
                                        + clss[i].toString() + "  "
                                        + flds[i].length + " must be "
                                        + cntFlds);
                    ...
                    for (int t = 0; t < flds[i].length; t++) {
                        if (flds[i][t].getName().length() < 65500) {
                            ReliabilityRunner
                                    .debug("Failed. Wrong length of field name in "
                                            + clss[i].toString()
                                            + "  "
            ...
            try {
                ...
                if (sm1 != sm2) {
                    ReliabilityRunner
                            .debug("Failed. Wrong sum of field values in classes: "
                                    + sm2 + " must be " + sm1);
