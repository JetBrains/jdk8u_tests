 ...
 */
public class EventTest008 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT008.EventDebuggee008";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT008/EventDebuggee008_Thread;";
     ...
     * and all expected 'METHOD_EXIT' events are received.
     */
    public Result  testEvent008() {

        logWriter.println("==> testEvent008: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee008.THREAD_NAME_PATTERN + i;
        ...
        if ( outOfMemory ) {
            if ( testCaseStatus == FAILURE ) {
                ...
                terminateDebuggee(SUCCESS, "MARKER_09.05");
                return passed
        ...
        if ( testCaseStatus == FAILURE ) {
            ...
            terminateDebuggee(FAILURE, "MARKER_09.06");
            return failed
            ("## FAILURE while sending requests for 'METHOD_ENTRY' and 'METHOD_EXIT' events");
        }
        ...
        if ( ! SIGNAL_READY_02.equals(debuggeeSignal) ) {
            terminateDebuggee(FAILURE, "MARKER_09.07");
            return failed
        ...
        logWriter.println("==> testEvent008: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent008: FAILED");
            return failed("testEvent008:");
        } else {
            logWriter.println("==> testEvent008: OK");
            return passed("testEvent008: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent008: FAILED");
    return failed("==> testEvent008: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest008().test(args));
	}
}
