 ...
 */
public class EventTest002 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.EVENT002.EventDebuggee002";
    ...
    public final static String THREAD_CLASS_SIGNATURE = 
        "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/EVENT002/EventDebuggee002_Thread;";
     ...
     * 'METHOD_EXIT' events are received.
     */
    public Result  testEvent002() {

        logWriter.println("==> testEvent002: START (" + new Date() + ")...");
...
try {
        ...
        for (int i=0; i < startedThreadsNumber; i++) {
            startedThreadsNames[i] = EventDebuggee002.THREAD_NAME_PATTERN + i;
        ...
        if ( methodEntryRequestsNumber != 0 ) {
            logWriter.println("\n");
        ...
        logWriter.println("==> testEvent002: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testEvent002: FAILED");
            return failed("testEvent002:");
        } else {
            logWriter.println("==> testEvent002: OK");
            return passed("testEvent002: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testEvent002: FAILED");
    return failed("==> testEvent002: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new EventTest002().test(args));
	}
}
