...
public class StringsTest3 extends MTTest {

    public StringsTest3(int threadId) {
        super(threadId);
    }
        ...
        try {
            int length = Integer.getInteger(
                    "org.apache.harmony.test.stress.jni."
                            + "strings.StringsTest3.length", 1000).intValue();
            int maxStrCount = Integer.getInteger(
                    "org.apache.harmony.test.stress.jni."
                            + "strings.StringsTest3.maxStrCount", 1000)
                    .intValue();
            ...
            if (res == 0) {
                testLog("Working thread",
                        "StringsTest3 FAILED: native method returned 0");
                ReliabilityRunner.debug("Test failed");
                ReliabilityRunner.mainTest.addError(this, new JNITestError());
            }
        } catch (Throwable t) {
            testLog("Working thread", "StringsTest3: ERROR: Exception caught:");
