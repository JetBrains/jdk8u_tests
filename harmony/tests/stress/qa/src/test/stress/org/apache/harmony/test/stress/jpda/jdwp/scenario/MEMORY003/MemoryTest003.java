...
/**
 * This test case exercises the JDWP command <code>VirtualMachine.AllClasses</code>
 * when very big number of classes are loaded in debuggee
 * and under memory stressing in debuggee.
 */
public class MemoryTest003 extends StressTestCase {
    
    public final static String DEBUGGEE_CLASS_NAME = 
        "org.apache.harmony.test.stress.jpda.jdwp.scenario.MEMORY003.MemoryDebuggee003";
     ...
     * and under memory stressing in debuggee.
     */
    public Result  testMemory003() {

        logWriter.println("==> testMemory003: START (" + new Date() + ")...");
...
try {
        ...
        if ( ! printExpectedError(reply, JDWPConstants.Error.OUT_OF_MEMORY,
            ...
            logWriter.println
            ("==> Number of reference types AFTER loading by debuggee new classes = " + newClasses);
        }
        ...
        if ( ! printExpectedError(reply, JDWPConstants.Error.OUT_OF_MEMORY,
            ...
            int newClasses = reply.getNextValueAsInt();
            logWriter.println
            ("==> Number of reference types AFTER creating by debugge memory stressing = " + newClasses);

            logWriter.println("\n");
            logWriter.println
            ("==> Check if all expected classes are returned by VirtualMachine::AllClasses command...");
            int sourceClasses = 6;
            int generatedClasses = sourceClasses * MemoryDebuggee003.ARRAY_DIMENSION;
            ...
            expectedClassSignatures[1] = 
                "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/MEMORY003/Memory003_TestClass01;";
            expectedClassSignatures[2] = 
                "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/MEMORY003/Memory003_TestClass02;";
            expectedClassSignatures[3] = 
                "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/MEMORY003/Memory003_TestClass03;";
            expectedClassSignatures[4] = 
                "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/MEMORY003/Memory003_TestClass04;";
            expectedClassSignatures[5] = 
                "Lorg/apache/harmony/test/stress/jpda/jdwp/scenario/MEMORY003/Memory003_TestClass05;";
            StressTestPrimitives.fillArrayOfSignatures
            (expectedClassSignatures, sourceClasses, MemoryDebuggee003.ARRAY_DIMENSION);
        ...
        logWriter.println("==> testMemory003: running time(mlsecs) = " + testRunTimeMlsec);
        if ( testCaseStatus == FAILURE ) {
            logWriter.println("==> testMemory003: FAILED");
            return failed("testMemory003:");
        } else {
            logWriter.println("==> testMemory003: OK");
            return passed("testMemory003: OK");
        }
} catch (Throwable thrown) {
    ...
    logWriter.println("==> testMemory003: FAILED");
    return failed("==> testMemory003: Unexpected Exception! ");
}
    }

	public static void main(String[] args) {
        System.exit(new MemoryTest003().test(args));
	}
}
