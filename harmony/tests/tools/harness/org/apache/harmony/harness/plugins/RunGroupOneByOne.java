...
public class RunGroupOneByOne extends ExecUnit implements Runtime, Positive {

    private final static String classID = "RunGroupOneByOne";
        ...
        if (tests != null && tests.size() > 0) {
            ...
            for (int i = 0; i < tests.size(); i++) {
                ...
                rt[i] = new RunTestThread(clas, curT, synchronizedObj,
                    testThGroup);
                try {
                    calcTimeout(curT.getTestTimeout());
                    rt[i].start();
                    int cnt;
                    for (cnt = 0; cnt < curTimeout; cnt++) {
                        rt[i].join(100);
                        if (!rt[i].isAlive()) {
                            break;
                        }
