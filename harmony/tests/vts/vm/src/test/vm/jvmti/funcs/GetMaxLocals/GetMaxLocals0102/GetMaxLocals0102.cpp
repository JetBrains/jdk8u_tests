...
const char test_case_name[] = "GetMaxLocals0102";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetMaxLocals((jmethodID)jvmti_env, &max);

    fprintf(stderr, "\tnative: GetMaxLocals result = %d (must be JVMTI_ERROR_INVALID_METHODID (23)) \n", result);
    fprintf(stderr, "\tnative: method ID is %p \n", method);
    fprintf(stderr, "\tnative: max is %d \n", max);
    fflush(stderr);
