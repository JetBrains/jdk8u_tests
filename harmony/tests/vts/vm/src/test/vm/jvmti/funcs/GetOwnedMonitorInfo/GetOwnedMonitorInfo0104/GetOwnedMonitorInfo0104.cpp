...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetOwnedMonitorInfo0104 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\n\tnative: GetOwnedMonitorInfo result = %d (must be zero) \n", result);

    result = jvmti_env->GetOwnedMonitorInfo(thread_1,
                &owned_monitor_count, &owned_monitors);

    fprintf(stderr, "\n\tnative: GetOwnedMonitorInfo result = %d (must be JVMTI_ERROR_THREAD_NOT_ALIVE (15))\n",
         result );
    if (result == JVMTI_ERROR_THREAD_NOT_ALIVE) {
        test = true;
        return;
    }
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetOwnedMonitorInfo0104         : ");
    ...
    fprintf(stderr, "\n} /* test GetOwnedMonitorInfo0104 is finished */ \n");
    fflush(stderr);
}
