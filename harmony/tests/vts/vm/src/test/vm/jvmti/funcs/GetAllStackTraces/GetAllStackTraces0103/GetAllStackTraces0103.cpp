...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetAllStackTraces0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetAllStackTraces(frame_count, NULL, &thread_count);
    fprintf(stderr, "\tnative: GetAllStackTraces result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n", result);
    fprintf(stderr, "\tnative: stack_info ptr is NULL (must be zero) \n");
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetAllStackTraces0103           : ");
    ...
    fprintf(stderr, "\n} /* test GetAllStackTraces0103 is finished */ \n");
    fflush(stderr);
}
