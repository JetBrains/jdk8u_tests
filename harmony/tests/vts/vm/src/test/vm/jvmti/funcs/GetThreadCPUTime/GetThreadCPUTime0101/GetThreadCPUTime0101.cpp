...
const char test_case_name[] = "GetThreadCPUTime0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetThreadCpuTime(thread, &nanos);
    fprintf(stderr, "\tnative: GetThreadCpuTime result = %d (must be zero) \n", result);
    if (result != JVMTI_ERROR_NONE) return;
    test = true;
    fprintf(stderr, "\tnative: thread   is %p \n", thread);
    fprintf(stderr, "\tnative: nanos    is %lld \n", (long long)nanos);
    if (nanos > 0) test = true;
    fflush(stderr);
