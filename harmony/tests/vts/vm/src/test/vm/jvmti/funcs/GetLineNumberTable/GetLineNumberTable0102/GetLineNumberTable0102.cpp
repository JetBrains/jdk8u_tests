...
const char test_case_name[] = "GetLineNumberTable0102";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    CAPABILITY_TURN_OFF_VOID(can_get_line_numbers);

    result = jvmti_env->GetLineNumberTable(method, &count, &table);
    fprintf(stderr, "\tnative: GetLineNumberTable result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
    fflush(stderr);
