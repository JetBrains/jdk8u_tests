...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetFrameCount0101 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetFrameCount(NULL, &count);
    fprintf(stderr, "\tnative: GetFrameCount result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: frame count is %d (must be 2)\n", count);

    if ((result == JVMTI_ERROR_NONE) && (count == 2))
        test = true;
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetFrameCount0101               : ");
    ...
    fprintf(stderr, "\n} /* test GetFrameCount0101 is finished */ \n");
    fflush(stderr);
}
