...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest SuspendThreadList0102 is started\n{\n");
    fflush(stderr);

    CAPABILITY_TURN_OFF(can_suspend);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\tnative: SuspendThreadList w/o capability \n");

    result = jvmti_env->SuspendThreadList(3, my_threads, results);

    fprintf(stderr, "\tnative: SuspendThreadList result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
    for ( i = 0; i < 3; i++)
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function SuspendThreadList0102           : ");
    fflush(stderr);
    ...
    fprintf(stderr, "\n} /* test SuspendThreadList0102 is finished */ \n");
    fflush(stderr);
}
