...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest IterateObjectsFromObject0102 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->IterateOverObjectsReachableFromObject( object,
            object_reference_callback, user_data);

    fprintf(stderr, "\tnative: IterateOverObjectsReachableFromObject result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n",
            result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function IterateOverObjectsReachableFromObject0102  : ");
    ...
    fprintf(stderr, "\n} /* test IterateObjectsFromObject0102 is finished */ \n");
    fflush(stderr);
}
