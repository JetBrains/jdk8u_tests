...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest SuspendThread0105 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->SuspendThread(thread_1);

    fprintf(stderr, "\tnative: SuspendThread result = %d (must be JVMTI_ERROR_THREAD_NOT_ALIVE (15)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function SuspendThread0105               : ");
    ...
    fprintf(stderr, "\n} /* test SuspendThread0105 is finished */ \n");
    fflush(stderr);
}
