...
const char test_case_name[] = "GetClassMethods0102";
...
void JNICALL callbackClassLoad(prms_CLS_LD)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassMethods0102;")) return;
    ...
    result = jvmti_env->GetClassStatus(klass, &status);
    fprintf(stderr, "\tnative: GetClassStatus result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: status is %d \n", status);
    fflush(stderr);
    if (result != JVMTI_ERROR_NONE) return;
    ...
    fprintf(stderr, "\tnative: GetClassMethods result = %d (must be JVMTI_ERROR_CLASS_NOT_PREPARED (22)) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: met_num is %d \n", met_num);
    fprintf(stderr, "\tnative: mids is %p \n", mids);
    fflush(stderr);
    if ( result == JVMTI_ERROR_CLASS_NOT_PREPARED ) test = true;
}
