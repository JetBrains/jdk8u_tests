...
const char test_case_name[] = "SetFieldAccessWatch0105";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "SetFieldAccessWatch0105.java", "first_field", &myclass, &myfield, DEBUG_OUT))
        return;

    result = jvmti_env->SetFieldAccessWatch(myclass, NULL);
    fprintf(stderr, "\tnative: SetFieldAccessWatch result = %d (must be JVMTI_ERROR_INVALID_FIELDID (25)) \n", result);
    fflush(stderr);
