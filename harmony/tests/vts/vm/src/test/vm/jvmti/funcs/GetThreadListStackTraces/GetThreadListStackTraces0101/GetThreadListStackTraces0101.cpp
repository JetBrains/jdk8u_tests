...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetThreadListStackTraces(tc, threads, fc, &sinfo);
    fprintf(stderr, "\tnative: GetThreadListStackTraces result = %d (must be zero) \n",
            result);

    if ((result == JVMTI_ERROR_NONE) && (sinfo != NULL))
        test = true;
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetThreadListStackTraces0101    : ");
