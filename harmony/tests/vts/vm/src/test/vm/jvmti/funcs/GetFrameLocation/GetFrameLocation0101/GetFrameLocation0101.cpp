...
/**
 * test of function GetFrameLocation
 */
...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetFrameLocation0101 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetFrameLocation(NULL, depth, &mid, &loc);
    fprintf(stderr, "\tnative: GetFrameLocation result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: current method ID is %p \n", method);
    fprintf(stderr, "\tnative: result method ID is %p (must be equal to current method ID)\n", mid);
    fprintf(stderr, "\tnative: result location is %lld (must be non-zero)\n", (long long)loc);

    if ((result == JVMTI_ERROR_NONE) && (mid == method)) test = true;
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetFrameLocation0101            : ");
    ...
    fprintf(stderr, "\n} /* test GetFrameLocation0101 is finished */ \n");
    fflush(stderr);
}
