...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetThreadGroupInfo0101 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    for (int i = 0; i < tcount; i++)
    {
        ...
        result = jvmti_env->GetThreadGroupInfo(tinfo.thread_group, &group_info);
        fprintf(stderr, "\tnative: GetThreadGroupInfo result = %d (must be zero) \n",
                result);
       ...
       {
           fprintf(stderr, "\tnative: group_info.name is %s \n",
                   group_info.name);
           fprintf(stderr, "\tnative: group_info.max_priority is %d \n",
                   group_info.max_priority);
           fprintf(stderr, "\tnative: group_info.is_daemon is %d \n",
                   group_info.is_daemon);
           if (strcmp(group_info.name, "First testing ThreadGroup"))
               return;
           if (group_info.max_priority != 10) return;
           if (group_info.is_daemon) return;
           test = true;
           fprintf(stderr, "\tnative: test is true\n");
       }
    }
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetThreadGroupInfo0101          : ");
    ...
    fprintf(stderr, "\n} /* test GetThreadGroupInfo0101 is finished */ \n");
    fflush(stderr);
}
