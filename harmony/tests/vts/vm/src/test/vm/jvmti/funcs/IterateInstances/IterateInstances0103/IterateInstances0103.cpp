...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest IterateInstances0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    fprintf(stderr, "\tnative: IterateOverInstancesOfClass result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n",
            result);

    if (result == JVMTI_ERROR_INVALID_CLASS)
        test = true;
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function IterateOverInstancesOfClass0103  : ");
    ...
    fprintf(stderr, "\n} /* test IterateInstances0103 is finished */ \n");
    fflush(stderr);
}
