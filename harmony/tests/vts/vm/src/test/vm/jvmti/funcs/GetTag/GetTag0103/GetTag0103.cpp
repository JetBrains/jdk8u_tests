...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetTag(NULL, &tag_g);
    fprintf(stderr, "\tnative: GetTag result = %d (must be JVMTI_ERROR_INVALID_OBJECT (20)) \n", result);
    fprintf(stderr, "\tnative: value is %p (must be NON-zero) \n", value);
    fprintf(stderr, "\tnative: tag_get is %lld \n", (long long)tag_g);

    if (result != JVMTI_ERROR_INVALID_OBJECT) return;
