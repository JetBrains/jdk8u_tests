...
const char test_case_name[] = "IsMethodSynthetic0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->IsMethodSynthetic(method, &synthetic);

    fprintf(stderr, "\tnative: IsMethodSynthetic result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: method ID is %p \n", method);
    fprintf(stderr, "\tnative: synthetic is %d \n", synthetic);
    fflush(stderr);

    if ((result == JVMTI_ERROR_NONE) && (!synthetic)) test = true;
}
