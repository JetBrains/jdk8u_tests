...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetTopThreadGroups0102 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->GetTopThreadGroups(NULL, &group_ptr);
    fprintf(stderr, "\tnative: GetTopThreadGroups result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetTopThreadGroups0102          : ");
    ...
    fprintf(stderr, "\n} /* test GetTopThreadGroups0102 is finished */ \n");
    fflush(stderr);
}
