...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetTopThreadGroups0101 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->GetTopThreadGroups(&group_counter, &group_ptr);
    fprintf(stderr, "\tnative: GetThreadInfo result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: group_counter is %d (must be >= 1) \n", group_counter);
    fprintf(stderr, "\tnative: group_ptr is %p (must be non-zero) \n", group_ptr);
    if ((result == JVMTI_ERROR_NONE) && (group_counter >= 1)) test = true;
}
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetTopThreadGroups0101          : ");
    ...
    fprintf(stderr, "\n} /* test GetTopThreadGroups0101 is finished */ \n");
    fflush(stderr);
}
