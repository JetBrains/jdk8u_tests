...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "ClearFieldAccessWatch0103.java", "first_field", &myclass, &myfield, DEBUG_OUT))
        return;

    result = jvmti_env->ClearFieldAccessWatch(myclass, myfield);
    fprintf(stderr, "\tnative: ClearFieldAccessWatch result = %d (must be JVMTI_ERROR_NOT_FOUND (41)) \n", result);
