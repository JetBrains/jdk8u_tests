...
const char test_case_name[] = "GetClassStatus0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassStatus0101;")) return;

    result = jvmti_env->GetClassStatus(myclass, &status);
    fprintf(stderr, "\tnative: GetClassStatus: result = %d\n", result);
    fprintf(stderr, "\tnative: status is %d\n", status);
    fprintf(stderr, "\tnative: status is ");
    class_status_output_as_string(status);
    fprintf(stderr, "\n"); fflush(stderr);
    if (result != JVMTI_ERROR_NONE) return;

    if (status == ethalon) test = true;
}
