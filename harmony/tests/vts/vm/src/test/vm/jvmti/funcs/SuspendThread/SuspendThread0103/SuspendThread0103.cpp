...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest SuspendThread0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\tnative: SuspendThread #1 result = %d (must be zero) \n", result);

    if (result != JVMTI_ERROR_NONE) return;

    result = jvmti_env->GetThreadState(my_thread, &state);
    fprintf(stderr, "\tnative: GetThreadState #1 result = %d (must be zero) \n", result);
    ...
    if (state == ethalon_state) {
        result = jvmti_env->SuspendThread(my_thread);
        fprintf(stderr, "\tnative: SuspendThread #2 result = %d (must be  JVMTI_ERROR_THREAD_SUSPENDED (14)) \n", result);
        if (result == JVMTI_ERROR_THREAD_SUSPENDED) test = true;
    }
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function SuspendThread0103               : ");
    ...
    fprintf(stderr, "\n} /* test SuspendThread0103 is finished */ \n");
    fflush(stderr);
}
