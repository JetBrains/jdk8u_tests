...
const char test_case_name[] = "GetFieldDeclaringClass0101";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetFieldDeclaringClass0101;"))
        return;
    ...
    result = jvmti_env->GetFieldDeclaringClass(klass, my_field, &declaring_class);
    fprintf(stderr, "\tnative: GetFieldDeclaringClass result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: fids is %p \n", my_field);
    fprintf(stderr, "\tnative: declaring_class ptr is %p \n", declaring_class);
    fflush(stderr);
    if ( result != JVMTI_ERROR_NONE ) return;

    res = jni_env->IsSameObject(klass, declaring_class);
    fprintf(stderr, "\tnative: res is %d \n", res);
    fflush(stderr);
