...
const char test_case_name[] = "ClearFieldAccessWatch0103";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassloaderClasses0103;")) return;
    fprintf(stderr, "\tnative: GetClassSignature result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: signature is %s \n", signature);
    fprintf(stderr, "\tnative: generic is %s \n", generic);
    ...
    result = jvmti_env->GetClassLoaderClasses(loader, NULL, &classes);
    fprintf(stderr, "\tnative: GetClassLoaderClasses result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n", result);
