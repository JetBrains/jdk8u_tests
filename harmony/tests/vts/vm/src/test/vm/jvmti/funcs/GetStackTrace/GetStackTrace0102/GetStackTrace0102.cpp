...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetStackTrace0102 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    fprintf(stderr, "\tnative: GetStackTrace with INVALID startDepth \n");
    result = jvmti_env->GetStackTrace(thread, -65536*1024, 1, frames, &count);
    fprintf(stderr, "\tnative: GetStackTrace result = %d (must be JVMTI_ERROR_ILLEGAL_ARGUMENT (103)) \n", result);
    fflush(stderr);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\nTest of function GetStackTrace0102               : ");
    ...
    fprintf(stderr, "\n} /* test GetStackTrace0102 is finished */ \n");
    fflush(stderr);
}
