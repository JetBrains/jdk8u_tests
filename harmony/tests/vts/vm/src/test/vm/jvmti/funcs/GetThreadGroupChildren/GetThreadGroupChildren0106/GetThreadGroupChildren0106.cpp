...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetThreadGroupChildren0106 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->GetThreadGroupChildren( tinfo.thread_group,
            &thread_counter, &thread_ptr, &group_counter, NULL);
    fprintf(stderr, "\tnative: GetThreadGroupChildren result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n",
            result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\nTest of function GetThreadGroupChildren0106      : ");
    ...
    fprintf(stderr, "\n} /* test GetThreadGroupChildren0106 is finished */ \n");
    fflush(stderr);
}
