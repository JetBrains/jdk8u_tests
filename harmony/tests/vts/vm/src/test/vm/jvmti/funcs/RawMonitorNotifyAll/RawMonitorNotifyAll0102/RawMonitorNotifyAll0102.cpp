...
const char test_case_name[] = "RawMonitorNotifyAll0102";
...
void JNICALL function(jvmtiEnv* jvmti_env, JNIEnv* jni_env, void* arg)
{
    ...
    if (entered < 5)
    {
        ...
        fprintf(stderr, "\tnative: thread -> (%d) start RawMonitorWait\n",
                current);
        result = jvmti_env->RawMonitorWait(mntr, 1000);
    ...
    if (5 == current)
    {
        ...
        fprintf(stderr, "\tnative: RawMonitorNotifyAll result = %d (must be JVMTI_ERROR_NOT_MONITOR_OWNER (51))\n", result);
        fprintf(stderr, "\tnative: current is %d \n", current);
        fprintf(stderr, "\tnative: thread 5 (%d) start RawMonitorExit\n",
                current);
        if (result == JVMTI_ERROR_NOT_MONITOR_OWNER)
        {
            test = true;
        }
        finish = true;
