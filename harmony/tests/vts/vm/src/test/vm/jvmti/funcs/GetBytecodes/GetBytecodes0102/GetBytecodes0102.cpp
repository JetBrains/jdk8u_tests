...
const char test_case_name[] = "GetBytecodes0102";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    CAPABILITY_TURN_OFF_VOID(can_get_bytecodes);

    result = jvmti_env->GetBytecodes(method, &count, &bytecodes);
    fprintf(stderr, "\tnative: GetBytecodes result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
    fflush(stderr);
