...
const char test_case_name[] = "GetObjectHashCode0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetObjectHashCode((jobject)thread, &hash_code );
    fprintf(stderr, "\tnative: GetObjectHashCode result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: thread is %p \n", thread);
    fprintf(stderr, "\tnative: hash_code is %lld \n", (long long)hash_code);
    fflush(stderr);

    if ((result == JVMTI_ERROR_NONE) && (hash_code != 0)) test = true;
}
