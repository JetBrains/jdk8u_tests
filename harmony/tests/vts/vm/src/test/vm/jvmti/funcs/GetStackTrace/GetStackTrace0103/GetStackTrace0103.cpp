...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetStackTrace0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    fprintf(stderr, "\tnative: GetStackTrace with INVALID thread\n");
    result = jvmti_env->GetStackTrace(thread_x, 0, 1, frames, &count);
    fprintf(stderr, "\tnative: GetStackTrace result = %d (must be JVMTI_ERROR_INVALID_THREAD (10)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\nTest of function GetStackTrace0103               : ");
    ...
    fprintf(stderr, "\n} /* test GetStackTrace0103 is finished */ \n");
    fflush(stderr);
}
