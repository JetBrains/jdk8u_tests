...
const char test_case_name[] = "SetFieldAccessWatch0101";
...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    Callbacks CB;
    check_AGENT_ONLOAD;
    jvmtiEvent events[] = { JVMTI_EVENT_EXCEPTION, JVMTI_EVENT_VM_DEATH, JVMTI_EVENT_FIELD_ACCESS };
    cb_exc;
    cb_acc;
    cb_death;
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "SetFieldAccessWatch0101.java", "first_field", &myclass, &myfield, DEBUG_OUT))
        return;
...
void JNICALL callbackFieldAccess(prms_FLD_ACCESS)
{
    ...
    test = true;
    fprintf(stderr, "\tnative: FieldAccess event was received\n");
