...
const char test_case_name[] = "ClearFieldModif0102";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "ClearFieldModif0102.java", "third_field", &myclass, &myfield, DEBUG_OUT))
        return;
    ...
    fprintf(stderr, "\tnative: ClearFieldModificationWatch result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
    fprintf(stderr, "\tnative: class is %p \n", myclass);
    fprintf(stderr, "\tnative: field is %p \n", myfield);
    fflush(stderr);
