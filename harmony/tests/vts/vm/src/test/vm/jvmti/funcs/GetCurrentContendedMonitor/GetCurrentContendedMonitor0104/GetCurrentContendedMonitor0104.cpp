...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetCurrentContendedMonitor0104 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->GetCurrentContendedMonitor(thread_1, &monitor);

    fprintf(stderr,
                "\tnative: GetCurrentContendedMonitor result = %d (must be JVMTI_ERROR_THREAD_NOT_ALIVE (15)) \n",
                result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetCurrentContendedMonitor0104  : ");
    ...
    fprintf(stderr, "\n} /* test GetCurrentContendedMonitor0104 is finished */ \n");
    fflush(stderr);
}
