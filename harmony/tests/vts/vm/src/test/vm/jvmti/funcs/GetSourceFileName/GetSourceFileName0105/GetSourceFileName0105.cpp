...
const char test_case_name[] = "GetSourceFileName0105";
...
void JNICALL callbackClassLoad(prms_CLS_LD)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetSourceFileName0105;")) return;
    ...
    result = jvmti_env->GetSourceFileName(klass, NULL);
    fprintf(stderr, "\tnative: GetSourceFileName with incorrect data (NULL-pointer to filename)\n");
    fprintf(stderr, "\tnative: GetSourceFileName result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n", result);
    fflush(stderr);
