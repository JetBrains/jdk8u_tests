...
const char test_case_name[] = "GetClassModifiers0101";
...
void JNICALL callbackClassLoad(prms_CLS_LD)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassModifiers0101;")) return;
    ...
    result = jvmti_env->GetClassModifiers(klass, &mods);
    fprintf(stderr, "\tnative: GetClassModifiers result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: modifiers are %d (must be 33)\n", mods);
    fflush(stderr);

    if ( result != JVMTI_ERROR_NONE) return;

    if (mods == 33) test = true;
}
