...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetCurrentContendedMonitor0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr,
            "\tnative: GetCurrentContendedMonitor with INVALID thread \n");
    result = jvmti_env->GetCurrentContendedMonitor(my_thread, &monitor);
    fprintf(stderr,
            "\tnative: GetCurrentContendedMonitor result = %d (must be JVMTI_ERROR_INVALID_THREAD (10)) \n",
            result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetCurrentContendedMonitor0103  : ");
    ...
    fprintf(stderr, "\n} /* test GetCurrentContendedMonitor0103 is finished */ \n");
    fflush(stderr);
}
