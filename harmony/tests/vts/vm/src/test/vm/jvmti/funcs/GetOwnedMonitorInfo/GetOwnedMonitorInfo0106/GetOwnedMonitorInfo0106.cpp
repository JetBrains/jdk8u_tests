...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetOwnedMonitorInfo0106 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    if (strcmp(tinfo.name, "Owned")) return;
    fprintf(stderr, "\tnative: test started\n");

    flag = true;
    util = true;

    fprintf(stderr, "\tnative: GetOwnedMonitorInfo owned_monitors = NULL \n");
    result = jvmti_env->GetOwnedMonitorInfo(thread, &owned_monitor_count, NULL);
    fprintf(stderr, "\tnative: GetOwnedMonitorInfo result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n", result);

    if (result == JVMTI_ERROR_NULL_POINTER)
    {
        test = true;
        return;
    }
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetOwnedMonitorInfo0106         : ");
    ...
    fprintf(stderr, "\n} /* test GetOwnedMonitorInfo0106 is finished */ \n");
    fflush(stderr);
}
