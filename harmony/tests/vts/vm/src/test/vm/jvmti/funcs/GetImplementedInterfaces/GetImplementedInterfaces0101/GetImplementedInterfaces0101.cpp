...
const char test_case_name[] = "GetImplementedInterfaces0101";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR) {
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/TestThread_C_09;")) return;
    ...
    result = jvmti_env->GetImplementedInterfaces(klass, &int_num, &ints);
    fprintf(stderr, "\tnative: GetImplementedInterfaces result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: class is %p \n", klass);
    fprintf(stderr, "\tnative: int_num is %d \n", int_num);
    fprintf(stderr, "\tnative: interfaces are %p \n", ints);
    fflush(stderr);
    if (result != JVMTI_ERROR_NONE) return;

    for (int i = 0; i < int_num; i++)
    {
        result = jvmti_env->GetClassSignature(ints[i], &signature, &generic);
        if (DEBUG_OUT)
        {
            fprintf(stderr, "\tnative: GetClassSignature result = %d (must be zero) \n",  result);
        ...
        if (strcmp(signature, "Ljava/lang/Runnable;")) continue;
        fprintf(stderr, "\tnative: GetClassSignature result = %d (must be zero) \n",  result);
        fprintf(stderr, "\tnative: classes ptr is %p \n", ints[i]);
        fprintf(stderr, "\tnative: signature is %s \n", signature);
        fprintf(stderr, "\tnative: generic is %s \n", generic);
