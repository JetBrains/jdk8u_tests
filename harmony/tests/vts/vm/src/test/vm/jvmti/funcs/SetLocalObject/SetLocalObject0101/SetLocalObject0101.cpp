...
const char test_case_name[] = "SetLocalObject0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    fprintf(stderr, "\tnative: SetLocalObject result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value for set is %p \n", value_1);
    if (result != JVMTI_ERROR_NONE) return;

    result = jvmti_env->GetLocalObject(NULL, depth, slot, &value_2);
    fprintf(stderr, "\tnative: GetLocalObject result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value after get is %p \n", value_2);
    if (result != JVMTI_ERROR_NONE) return;

    if (jni_env->IsSameObject(value_1, value_2)) test = true;
}
