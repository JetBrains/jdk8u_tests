...
const char test_case_name[] = "ClearFieldAccessWatch0101";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassloaderClasses0101;")) return;
    fprintf(stderr, "\tnative: GetClassSignature result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: signature is %s \n", signature);
    fprintf(stderr, "\tnative: generic is %s \n", generic);
    ...
    result = jvmti_env->GetClassLoader(klass, &loader);
    fprintf(stderr, "\tnative: GetClassLoader result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: loader is %p \n", loader);
    fflush(stderr);
    if ( result != JVMTI_ERROR_NONE ) return;
    ...
    result = jvmti_env->GetClassLoaderClasses(loader, &cc, &classes);
    fprintf(stderr, "\tnative: GetClassLoaderClasses result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: classes ptr is %p \n", klass);
    fprintf(stderr, "\tnative: class counter is %d \n", cc);
    fprintf(stderr, "\tnative: loader is %p \n", loader);
    fflush(stderr);
    if ( result != JVMTI_ERROR_NONE) return;
    ...
    for (int i = 0; i < cc; i++)
    {
        ...
        if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassloaderClasses0101;")) continue;
