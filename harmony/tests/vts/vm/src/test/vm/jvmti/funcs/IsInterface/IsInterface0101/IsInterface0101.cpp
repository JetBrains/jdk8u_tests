...
const char test_case_name[] = "IsInterface0101";
...
void JNICALL callbackClassLoad(prms_CLS_LD)
{
    ...
    flag = true;
    util = true;
    result = jvmti_env->IsInterface(klass, &res);
    fprintf(stderr, "\tnative: IsInterface result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass is %p \n", klass);
    fprintf(stderr, "\tnative: is interface: %d \n", res);
    fflush(stderr);

    if ((result == JVMTI_ERROR_NONE) && (res)) test = true;
}
