...
const char test_case_name[] = "SetLocalLong0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    for (int i = 0; i < varnum; i++)
        if (!strcmp(table[i].name, "_LNG"))
            slot = table[i].slot;

    result = jvmti_env->SetLocalLong(NULL, depth, slot, value_1);
    fprintf(stderr, "\tnative: SetLocalLong result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value for set is %lld (must be 555) \n", (long long)value_1);
    if (result != JVMTI_ERROR_NONE) return;

    result = jvmti_env->GetLocalLong(NULL, depth, slot, &value_2);
    fprintf(stderr, "\tnative: GetLocalLong result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value after get %lld (must be 555) \n", (long long)value_2);
    if (result != JVMTI_ERROR_NONE) return;

    if (value_1 == value_2) test = true;
}
