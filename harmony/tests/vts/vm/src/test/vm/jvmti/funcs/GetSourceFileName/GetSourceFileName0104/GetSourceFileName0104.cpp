...
const char test_case_name[] = "GetSourceFileName0104";
...
void JNICALL callbackClassLoad(prms_CLS_LD)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetSourceFileName0104;")) return;
    ...
    result = jvmti_env->GetSourceFileName((jclass)thread, &filename);
    fprintf(stderr, "\tnative: GetSourceFileName with incorrect data (INVALID_CLASS)\n");
    fprintf(stderr, "\tnative: GetSourceFileName result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n", result);
    fflush(stderr);
