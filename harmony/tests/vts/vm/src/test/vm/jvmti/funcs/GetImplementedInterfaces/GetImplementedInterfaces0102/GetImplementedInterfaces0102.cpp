...
const char test_case_name[] = "GetImplementedInterfaces0102";
...
void JNICALL callbackClassLoad(prms_CLS_LD) {
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/TestThread_C_09_02;")) return;
    ...
    fprintf(stderr, "\tnative: GetImplementedInterfaces result = %d (must be JVMTI_ERROR_CLASS_NOT_PREPARED (22)) \n", result);
    fflush(stderr);
    if (result != JVMTI_ERROR_CLASS_NOT_PREPARED ) return;
    test = true;
}
