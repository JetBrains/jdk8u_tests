...
const char test_case_name[] = "GetFieldModifiers0102";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetFieldModifiers0102;")) return;
    ...
    {
        result = jvmti_env->GetFieldName(klass, fids[i],
    ...
    result = jvmti_env->GetFieldModifiers((jclass)thread, my_fid, &modifiers);
    fprintf(stderr, "\tnative: GetFieldModifiers result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n", result);
    fflush(stderr);
