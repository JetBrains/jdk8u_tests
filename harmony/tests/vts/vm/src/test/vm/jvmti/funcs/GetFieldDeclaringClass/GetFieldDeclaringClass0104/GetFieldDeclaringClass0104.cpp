...
const char test_case_name[] = "GetFieldDeclaringClass0104";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetFieldDeclaringClass0104;"))
        return;
    ...
    result = jvmti_env->GetFieldDeclaringClass(klass, my_field, NULL);
    fprintf(stderr, "\tnative: GetFieldDeclaringClass result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n", result);
    fflush(stderr);
