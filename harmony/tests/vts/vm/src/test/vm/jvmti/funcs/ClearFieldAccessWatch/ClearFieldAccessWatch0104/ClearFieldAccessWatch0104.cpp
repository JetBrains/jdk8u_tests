...
const char test_case_name[] = "ClearFieldAccessWatch0104";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "ClearFieldAccessWatch0104.java", "first_field", &myclass, &myfield, DEBUG_OUT))
        return;
    ...
    result = jvmti_env->ClearFieldAccessWatch(NULL, myfield);
    fprintf(stderr, "\tnative: ClearFieldAccessWatch result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n", result);
    fprintf(stderr, "\tnative: class is NULL \n");
    fprintf(stderr, "\tnative: field is %p \n", myfield);
    fflush(stderr);
