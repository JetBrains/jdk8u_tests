...
const char test_case_name[] = "RawMonitorEnter0102";
...
void JNICALL callbackVMInit(prms_VMINIT)
{
    ...
    result = jvmti_env->RawMonitorEnter(NULL);
    fprintf(stderr, "\tnative: RawMonitorEnter result = %d (must be JVMTI_ERROR_INVALID_MONITOR (50)) \n", result);
    fprintf(stderr, "\tnative: name is %s \n", name);
    fprintf(stderr, "\tnative: monitor is %p \n", monitor);
    fflush(stderr);
