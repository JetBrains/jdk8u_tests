...
const char test_case_name[] = "GetFieldDeclaringClass0102";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetFieldDeclaringClass0102;")) return;
    ...
    result = jvmti_env->GetFieldDeclaringClass((jclass)thread, my_field, &declaring_class);
    fprintf(stderr, "\tnative: GetFieldDeclaringClass result = %d (must be JVMTI_ERROR_INVALID_CLASS) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
