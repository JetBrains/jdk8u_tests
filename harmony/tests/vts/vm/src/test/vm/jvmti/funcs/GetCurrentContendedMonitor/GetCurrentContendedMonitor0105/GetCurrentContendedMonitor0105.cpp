...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetCurrentContendedMonitor0105 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->GetCurrentContendedMonitor(thread, NULL);

    fprintf(stderr,
            "\tnative: GetCurrentContendedMonitor result = %d (must be JVMTI_ERROR_NULL_POINTER (100)) \n",
            result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetCurrentContendedMonitor0105  : ");
    ...
    fprintf(stderr, "\n} /* test GetCurrentContendedMonitor0105 is finished */ \n");
    fflush(stderr);
}
