...
const char test_case_name[] = "GetFieldName0103";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetFieldName0103;")) return;
	...
	// fieldID is NULL
    result = jvmti_env->GetFieldName(klass, NULL,
            &name, &signature, &generic);
    if (result !=  JVMTI_ERROR_NONE)  test = true;

    fprintf(stderr, "\tnative: GetFieldName result = %d (must be JVMTI_ERROR_INVALID_FIELDID) \n", result);
    fflush(stderr);
