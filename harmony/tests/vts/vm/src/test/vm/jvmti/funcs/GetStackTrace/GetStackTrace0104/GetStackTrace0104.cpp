...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetStackTrace0104 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetStackTrace(thread_1, 0, 1, frames, &count);
    fprintf(stderr, "\tnative: GetStackTrace result = %d (must be JVMTI_ERROR_THREAD_NOT_ALIVE (15)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\nTest of function GetStackTrace0104               : ");
    ...
    fprintf(stderr, "\n} /* test GetStackTrace0104 is finished */ \n");
    fflush(stderr);
}
