...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest SetThreadLocalStorage0101 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadEnd(prms_THRD_END)
{
    ...
    result = jvmti_env->GetThreadLocalStorage(thread, &data);

    fprintf(stderr, "\tnative: GetThreadLocalStorage result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: Local Storage ptr (get) is %p (must be non-zero)\n", data);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function SetThreadLocalStorage0101       : ");
    ...
    fprintf(stderr, "\n} /* test SetThreadLocalStorage0101 is finished */ \n");
    fflush(stderr);
}
