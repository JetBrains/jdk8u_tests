...
const char test_case_name[] = "GetFieldDeclaringClass0103";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetFieldDeclaringClass0103;")) return;
    ...
    result = jvmti_env->GetFieldDeclaringClass(klass, (jfieldID)100, &declaring_class);
    fprintf(stderr, "\tnative: GetFieldDeclaringClass result = %d (must be JVMTI_ERROR_INVALID_FIELDID (25)) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: fid is 100 \n");
