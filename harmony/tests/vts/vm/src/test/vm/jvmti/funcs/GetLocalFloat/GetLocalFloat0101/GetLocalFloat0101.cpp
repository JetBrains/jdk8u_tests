...
const char test_case_name[] = "GetLocalFloat0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetLocalFloat(NULL, depth, slot, &value);

    fprintf(stderr, "\tnative: GetLocalFloat result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value is %f (must be 0.2) \n", value);

    if ((result == JVMTI_ERROR_NONE) && (fabs(value - 0.1f) < 0.0000001)) test = true;
}
