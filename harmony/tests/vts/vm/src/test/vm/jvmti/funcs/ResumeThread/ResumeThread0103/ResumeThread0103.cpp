...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\tnative: ResumeThread NON suspended thread\n");
    result = jvmti_env->ResumeThread(my_thread);
    fprintf(stderr, "\tnative: ResumeThread result = %d (must be JVMTI_ERROR_THREAD_NOT_SUSPENDED (13)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function ResumeThread0103                : ");
    ...
    fprintf(stderr, "\n} /* test ResumeThread0103 is finished */ \n");
    fflush(stderr);
}
