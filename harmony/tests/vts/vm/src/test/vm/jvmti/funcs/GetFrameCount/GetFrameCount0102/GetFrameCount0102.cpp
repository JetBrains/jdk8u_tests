...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetFrameCount0102 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetFrameCount(thread_x, &count);
    fprintf(stderr, "\tnative: GetFrameCount result = %d (must be JVMTI_ERROR_INVALID_THREAD(10)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetFrameCount0102               : ");
    ...
    fprintf(stderr, "\n} /* test GetFrameCount0102 is finished */ \n");
    fflush(stderr);
}
