 ...
 *
 */
public class InterruptThread0101 {
    ...
    static public void special_method() {        
        TestThread_T_08 thread = new TestThread_T_08("SuperPuperTest");
        thread.start();
        ...
        new Thread ("agent") {
            public void run() {
                InterruptThread0101.start = true;
                return;                
            }
        }.start();
    ...
    public static void killThread(TestThread_T_08 thread) {
        thread.stop = true;
        try {
            thread.join();
        } catch (Throwable tex) {
...
class TestThread_T_08 extends Thread {

    boolean stop = false;
    
    TestThread_T_08(String name) {
        super(name);
    }    
