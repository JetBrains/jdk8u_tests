...
const char test_case_name[] = "SetLocalInt0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->SetLocalInt(NULL, depth, slot, value_1);
    fprintf(stderr, "\tnative: SetLocalInt result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value for set is %d (must be 555) \n", value_1);
    if (result != JVMTI_ERROR_NONE) return;

    result = jvmti_env->GetLocalInt(NULL, depth, slot, &value_2);
    fprintf(stderr, "\tnative: GetLocalInt result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value after get %d (must be 555) \n", value_2);
    if (result != JVMTI_ERROR_NONE) return;
