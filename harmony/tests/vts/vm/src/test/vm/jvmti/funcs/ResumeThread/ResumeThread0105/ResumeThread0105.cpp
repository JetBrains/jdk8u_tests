...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\tnative: ResumeThread with bad thread \n");
    result = jvmti_env->ResumeThread(my_thread);
    fprintf(stderr, "\tnative: ResumeThread result = %d (must be JVMTI_ERROR_INVALID_THREAD (10)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function ResumeThread0105                : ");
    ...
    fprintf(stderr, "\n} /* test ResumeThread0105 is finished */ \n");
    fflush(stderr);
}
