...
const char test_case_name[] = "GetClassFields0103";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetClassFields0103;")) return;
    ...
    result = jvmti_env->GetClassFields((jclass)thread, &fld_num, &fids);
    fprintf(stderr, "\tnative: GetClassFields result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: fld_num is %d \n", fld_num);
    fprintf(stderr, "\tnative: flds is %p \n", fids);
    fflush(stderr);
