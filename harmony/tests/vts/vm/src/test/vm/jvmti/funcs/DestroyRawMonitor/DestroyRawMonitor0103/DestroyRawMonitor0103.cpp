...
const char test_case_name[] = "DestroyRawMonitor0103";
...
void JNICALL callbackVMInit(prms_VMINIT)
{
    ...
    result = jvmti_env->DestroyRawMonitor((jrawMonitorID)jvmti_env);
    fprintf(stderr, "\tnative: DestroyRawMonitor result = %d (must be JVMTI_ERROR_INVALID_MONITOR (50)) \n", result);
    fflush(stderr);
