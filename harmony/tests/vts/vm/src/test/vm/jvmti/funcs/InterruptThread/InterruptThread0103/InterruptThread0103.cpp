...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest InterruptThread0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->InterruptThread(NULL);
    fprintf(stderr, "\tnative: InterruptThread result = %d (must be JVMTI_ERROR_INVALID_THREAD (10)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function InterruptThread0103             : ");
    ...
    fprintf(stderr, "\n} /* test InterruptThread0103 is finished */ \n");
    fflush(stderr);
}
