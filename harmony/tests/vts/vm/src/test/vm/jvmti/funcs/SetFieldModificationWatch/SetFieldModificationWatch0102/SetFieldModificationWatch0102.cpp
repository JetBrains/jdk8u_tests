...
const char test_case_name[] = "SetFieldModificationWatch0102";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "SetFieldModificationWatch0102.java", "third_field",
                &myclass, &myfield, DEBUG_OUT)) return;

    CAPABILITY_TURN_OFF_VOID(can_generate_field_modification_events);

    result = jvmti_env->SetFieldModificationWatch(myclass, myfield);
    fprintf(stderr, "\tnative: SetFieldModificationWatch result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
