...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetAllStackTraces0102 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetAllStackTraces(frame_count, &stack_info, &thread_count);
    fprintf(stderr, "\tnative: GetAllStackTraces result = %d (must be JVMTI_ERROR_ILLEGAL_ARGUMENT (103)) \n", result);
    fprintf(stderr, "\tnative: stack_info ptr is %p (must be zero) \n", stack_info);
    fprintf(stderr, "\tnative: thread_count is %d\n", thread_count);
    fflush(stderr);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetAllStackTraces0102           : ");
    ...
    fprintf(stderr, "\n} /* test GetAllStackTraces0102 is finished */ \n");
    fflush(stderr);
}
