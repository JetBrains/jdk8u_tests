...
const char test_case_name[] = "GetThreadCPUTimerInfo0102";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    CAPABILITY_TURN_OFF_VOID(can_get_thread_cpu_time);

    result = jvmti_env->GetThreadCpuTimerInfo(&info);
    fprintf(stderr, "\tnative: GetThreadCpuTimerInfo result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
