...
const char test_case_name[] = "GetMethodDeclaringClass0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetMethodDeclaringClass( method, &decl_class );
    fprintf(stderr, "\tnative: GetMethodDeclaringClass result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: method is %p \n", method);
    fprintf(stderr, "\tnative: decl_class is %p \n", decl_class);
    fflush(stderr);
    if ( result != JVMTI_ERROR_NONE) return;

    result = jvmti_env->GetClassSignature(decl_class, &signature,&generic);
    if (result != JVMTI_ERROR_NONE) return;
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetMethodDeclaringClass0101;"))
        return;
    fprintf(stderr, "\tnative: GetClassSignature result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: decl_class ptr is %p \n", decl_class);
