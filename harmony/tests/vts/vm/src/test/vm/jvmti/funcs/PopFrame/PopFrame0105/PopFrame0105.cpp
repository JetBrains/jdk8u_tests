...
const char test_case_name[] = "PopFrame0105";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (result == JVMTI_ERROR_NONE)
    {
        ...
        fprintf(stderr, "\tnative: PopFrame result = %d (must be zero) \n", result);
        ...
        if (result != JVMTI_ERROR_NONE)
        {
            fprintf(stderr, "\tnative: ResumeThread error\n");
        }
    }

    fprintf(stderr, "\tnative: ----- 2 call -----\n");
    ...
    {
        result = jvmti_env->PopFrame(my_thread);
        fprintf(stderr, "\tnative: PopFrame result = %d (must be JVMTI_ERROR_NO_MORE_FRAMES (31)) \n", result);
        fprintf(stderr, "\tnative: thread is %p (must be NON-zero) \n", thread);
        fflush(stderr);
