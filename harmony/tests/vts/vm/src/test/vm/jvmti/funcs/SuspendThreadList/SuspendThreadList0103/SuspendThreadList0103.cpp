...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest SuspendThreadList0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\tnative: SuspendThreadList request_count < 0\n");

    result = jvmti_env->SuspendThreadList(-1, my_threads, results);

    fprintf(stderr, "\tnative: SuspendThreadList result = %d (must be JVMTI_ERROR_ILLEGAL_ARGUMENT (103)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function SuspendThreadList0103           : ");
    fflush(stderr);
    ...
    fprintf(stderr, "\n} /* test SuspendThreadList0103 is finished */ \n");
    fflush(stderr);
}
