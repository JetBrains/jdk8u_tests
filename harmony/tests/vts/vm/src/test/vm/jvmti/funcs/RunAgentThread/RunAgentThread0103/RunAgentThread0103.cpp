...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest RunAgentThread0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->RunAgentThread(thread_x, proc, NULL, JVMTI_THREAD_MAX_PRIORITY);

    fprintf(stderr, "\tnative: RunAgentThread result = %d (must be JVMTI_ERROR_INVALID_THREAD (10)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function RunAgentThread0103              : ");
    ...
    fprintf(stderr, "\n} /* test RunAgentThread0103 is finished */ \n");
    fflush(stderr);
}
