...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest ResumeThreadList0103 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    result = jvmti_env->ResumeThreadList(-1, my_threads, results);
    fprintf(stderr, "\tnative: ResumeThreadList result = %d (must be JVMTI_ERROR_ILLEGAL_ARGUMENT (103)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\nTest of function ResumeThreadList0103           : ");
    ...
    fprintf(stderr, "\n} /* test ResumeThreadList0103 is finished */ \n");
    fflush(stderr);
}
