...
const char test_case_name[] = "ClearFieldModif0105";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "ClearFieldModif0105.java", "third_field", &myclass, &myfield, DEBUG_OUT))
        return;
    ...
    result = jvmti_env->ClearFieldModificationWatch(myclass, NULL);
    fprintf(stderr, "\tnative: ClearFieldModificationWatch result = %d (must be JVMTI_ERROR_INVALID_FIELDID (25)) \n", result);
    fprintf(stderr, "\tnative: class is %p \n", myclass);
