...
void JNICALL callbackVMInit(prms_VMINIT)
{
    ...
    fprintf(stderr, "\n\tnative: Allocate for correct size > 0\n");
    fprintf(stderr, "\tnative: Allocate result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: mem_ptr after Allocate is %p (must be NON-zero)\n", mem_ptr);
    fprintf(stderr, "\tnative: size after Allocate is %lld (must be NON-zero)\n", size);
    fflush(stderr);
