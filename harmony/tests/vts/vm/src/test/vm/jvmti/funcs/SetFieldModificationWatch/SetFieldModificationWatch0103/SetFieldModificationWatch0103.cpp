...
const char test_case_name[] = "SetFieldModificationWatch0103";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "SetFieldModificationWatch0103.java", "third_field", &myclass, &myfield, DEBUG_OUT))
        return;

    result = jvmti_env->SetFieldModificationWatch(myclass, myfield);
    fprintf(stderr, "\tnative: SetFieldModificationWatch #1 result = %d (must be zero) \n", result);
    ...
    result = jvmti_env->SetFieldModificationWatch(myclass, myfield);
    fprintf(stderr, "\tnative: SetFieldModificationWatch #2 result = %d (must be JVMTI_ERROR_DUPLICATE (40)) \n", result);
    fprintf(stderr, "\tnative: class is %p \n", myclass);
    fprintf(stderr, "\tnative: field is %p \n", myfield);
    fflush(stderr);
