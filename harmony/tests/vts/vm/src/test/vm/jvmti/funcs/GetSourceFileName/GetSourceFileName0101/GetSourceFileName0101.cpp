...
const char test_case_name[] = "GetSourceFileName0101";
...
void JNICALL callbackClassLoad(prms_CLS_LD)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/GetSourceFileName0101;")) return;
    ...
    fprintf(stderr, "\tnative: GetSourceFileName with correct data\n");
    fprintf(stderr, "\tnative: GetSourceFileName result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: klass ptr is %p \n", klass);
    fprintf(stderr, "\tnative: filename is %s \n", filename);
    fflush(stderr);
    if (result != JVMTI_ERROR_NONE) return;

    if (strcmp(filename, "GetSourceFileName0101.java")) return;
    test = true;
}
