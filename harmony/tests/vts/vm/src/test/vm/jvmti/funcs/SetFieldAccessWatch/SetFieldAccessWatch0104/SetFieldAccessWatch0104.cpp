...
const char test_case_name[] = "SetFieldAccessWatch0104";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "SetFieldAccessWatch0104.java", "first_field", &myclass, &myfield, DEBUG_OUT))
        return;

    result = jvmti_env->SetFieldAccessWatch(NULL, myfield);
    fprintf(stderr, "\tnative: SetFieldAccessWatch result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n", result);
    fflush(stderr);
