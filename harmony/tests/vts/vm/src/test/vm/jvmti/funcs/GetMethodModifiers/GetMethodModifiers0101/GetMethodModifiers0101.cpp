...
const char test_case_name[] = "GetMethodModifiers0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    result = jvmti_env->GetMethodModifiers(method, &modifiers);
    fprintf(stderr, "\tnative: GetMethodModifiers result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: method ID is %p \n", method);
    fprintf(stderr, "\tnative: modifiers is %d (must be 10)\n", modifiers);
    fflush(stderr);
    if (result != JVMTI_ERROR_NONE) return;
    if (modifiers == 10) test = true;
}
