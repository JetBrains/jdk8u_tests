...
const char test_case_name[] = "RawMonitorNotify0103";
...
void JNICALL function(jvmtiEnv* jvmti_env, JNIEnv* jni_env, void* arg)
{
    ...
    if (2 == current)
    {
        ...
        /*
         * In this place current thread must not be owner of monitor.
         */
        result = jvmti_env->RawMonitorNotify(mntr);
        if (result == JVMTI_ERROR_NOT_MONITOR_OWNER) test = true;

        fprintf(stderr, "\tnative: RawMonitorNotify result = %d (must be JVMTI_ERROR_NOT_MONITOR_OWNER (51))\n", result);
