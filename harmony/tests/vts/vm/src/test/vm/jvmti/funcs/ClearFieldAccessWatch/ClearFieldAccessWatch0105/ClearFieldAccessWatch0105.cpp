...
const char test_case_name[] = "ClearFieldAccessWatch0105";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    if (!is_needed_field_found(jvmti_env, "ClearFieldAccessWatch0105.java", "first_field", &myclass, &myfield, DEBUG_OUT))
        return;
    ...
    result = jvmti_env->ClearFieldAccessWatch(myclass, NULL);
    fprintf(stderr, "\tnative: ClearFieldAccessWatch result = %d (must be JVMTI_ERROR_INVALID_FIELDID (25)) \n", result);
    fprintf(stderr, "\tnative: class is %p \n", myclass);
    fprintf(stderr, "\tnative: field is NULL \n");
    fflush(stderr);
    if (result != JVMTI_ERROR_INVALID_FIELDID) return;
