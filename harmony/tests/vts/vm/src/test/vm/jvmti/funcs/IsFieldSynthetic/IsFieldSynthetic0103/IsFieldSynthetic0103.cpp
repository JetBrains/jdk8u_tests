...
const char test_case_name[] = "IsFieldSynthetic0103";
...
void JNICALL callbackClassPrepare(prms_CLS_PRPR)
{
    ...
    if (strcmp(signature, "Lorg/apache/harmony/vts/test/vm/jvmti/IsFieldSynthetic0103;"))
        return;
    ...
    result = jvmti_env->IsFieldSynthetic((jclass)thread, my_fid, &is_synthetic);

    if (result ==  JVMTI_ERROR_INVALID_CLASS) test = true;

    fprintf(stderr, "\tnative: IsFieldSynthetic result = %d (must be JVMTI_ERROR_INVALID_CLASS (21)) \n", result);
    fflush(stderr);
