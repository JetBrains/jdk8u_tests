...
const char test_case_name[] = "SetLocalFloat0101";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    fprintf(stderr, "\tnative: SetLocalFloat result = %d (must be zero) \n", result);
    fprintf(stderr, "\tnative: value for set is %lld (must be 55) \n", (long long)value_1);
    if (result != JVMTI_ERROR_NONE) return;

    result = jvmti_env->GetLocalFloat(NULL, depth, slot, &value_2);
