...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest GetCurrentContendedMonitor0102 is started\n{\n");
    fflush(stderr);

    CAPABILITY_TURN_OFF(can_get_current_contended_monitor);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    for (int i = 0; i < tcount; i++)
    {
        ...
        CAPABILITY_CHECK();
        fprintf(stderr, "\tnative: GetCurrentContendedMonitor w/o capability \n");
        result = jvmti_env->GetCurrentContendedMonitor(threads[i], &monitor);
        fprintf(stderr,
                "\tnative: GetCurrentContendedMonitor result = %d (must be JVMTI_ERROR_MUST_POSSESS_CAPABILITY (99)) \n", result);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n\tTest of function GetCurrentContendedMonitor0102  : ");
    ...
    fprintf(stderr, "\n} /* test GetCurrentContendedMonitor0102 is finished */ \n");
    fflush(stderr);
}
