...
JNIEXPORT jint JNICALL Agent_OnLoad(prms_AGENT_ONLOAD)
{
    ...
    fprintf(stderr, "\n-------------------------------------------------\n");
    fprintf(stderr, "\ntest ThreadStart0101 is started\n{\n");
    fflush(stderr);
...
void JNICALL callbackThreadStart(prms_THRD_START)
{
    ...
    fprintf(stderr, "\tnative: ThreadStart (prms) jvmti_env is %p\n", jvmti_env);
    fprintf(stderr, "\tnative: ThreadStart (prms) jni_env   is %p\n", jni_env);
    fprintf(stderr, "\tnative: ThreadStart (prms) thread    is %p\n", thread);
    fflush(stderr);
...
void JNICALL callbackVMDeath(prms_VMDEATH)
{
    ...
    fprintf(stderr, "\n} /* test ThreadStart0101 is finished */ \n");
    fflush(stderr);
}
