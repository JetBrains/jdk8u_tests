...
static jshort callNI(JNIEnv *env, jclass par_cl, jmethodID mid, ...)
{
    va_list args;
        jshort result;

        va_start(args, mid);
        result = (*env)->CallStaticShortMethodV(env, par_cl, mid, args);
        va_end(args);
        return result;
}

/*
 * Method: org.apache.harmony.vts.test.vm.jni.static_methods.CallStaticShortMethodVTest.nativeExecute(Lorg/apache/harmony/vts/test/vm/jni/static_methods/TestClass;Lorg/apache/harmony/vts/test/vm/jni/static_methods/NativeTestClass;)Z
 */
JNIEXPORT jboolean JNICALL
Java_org_apache_harmony_vts_test_vm_jni_static_1methods_CallStaticShortMethodVTest_nativeExecute
    (JNIEnv *env, jobject this_object, jobject tc, jobject ntc)
{
    ...
    result5 = callNI(env, class1, m5, so5);
    result6 = callNI(env, class1, m6, so5, so62);
    ...
    nresult5 = callNI(env, class2, nm5, so5);
    nresult6 = callNI(env, class2, nm6, so5, so62);
