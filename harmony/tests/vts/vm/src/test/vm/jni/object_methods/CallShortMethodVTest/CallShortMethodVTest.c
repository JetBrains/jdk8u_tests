...
static jshort callNI(JNIEnv *env, jobject obj, jmethodID mid, ...)
{
    va_list args;
        jshort result;

        va_start(args, mid);
        result = (*env)->CallShortMethodV(env, obj, mid, args);
        va_end(args);
        return result;
}

/*
 * Method: org.apache.harmony.vts.test.vm.jni.object_methods.CallShortMethodVTest.nativeExecute(Lorg/apache/harmony/vts/test/vm/jni/object_methods/TestClass;Lorg/apache/harmony/vts/test/vm/jni/object_methods/NativeTestClass;)Z
 */
JNIEXPORT jboolean JNICALL
Java_org_apache_harmony_vts_test_vm_jni_object_1methods_CallShortMethodVTest_nativeExecute
    (JNIEnv *env, jobject this_object, jobject tc, jobject ntc)
{
    ...
    result5 = callNI(env, tc, m5, so5);
    result6 = callNI(env, tc, m6, so5, so62);
    ...
    nresult5 = callNI(env, ntc, nm5, so5);
    nresult6 = callNI(env, ntc, nm6, so5, so62);
