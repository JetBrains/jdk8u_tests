...
import org.apache.harmony.tests.java.awt.Tools;
...
public class GeneralPathTest extends ShapeTestCase {
    ...
    public void testConstants() {
        Assert.assertEquals("WIND_EVEN_ODD", PathIterator.WIND_EVEN_ODD, GeneralPath.WIND_EVEN_ODD);
        Assert.assertEquals("WIND_NON_ZERO", PathIterator.WIND_NON_ZERO, GeneralPath.WIND_NON_ZERO);
    }

    public void testGetWindingRule() {
        Assert.assertEquals("Rule", GeneralPath.WIND_EVEN_ODD, g.getWindingRule());
    }

    public void testSetWindingRule() {
        g.setWindingRule(GeneralPath.WIND_NON_ZERO);
        Assert.assertEquals("Rule", GeneralPath.WIND_NON_ZERO, g.getWindingRule());
    ...
    public void testIllegalPathStateException() {
        try {
            g.lineTo(10, 20);
            Assert.fail("GeneralPath.lineTo() should throw exception IllegalPathStateException");
        } catch (IllegalPathStateException e) {
        }
        try {
            g.quadTo(10, 20, 30, 40);
            Assert.fail("GeneralPath.quadTo() should throw exception IllegalPathStateException");
        } catch (IllegalPathStateException e) {
        }
        try {
            g.curveTo(10, 20, 30, 40, 50, 60);
            Assert.fail("GeneralPath.curveTo() should throw exception IllegalPathStateException");
        } catch (IllegalPathStateException e) {
        }
        try {
            g.closePath();
            Assert.fail("GeneralPath.closePath() should throw exception IllegalPathStateException");
        } catch (IllegalPathStateException e) {
        }
    }
    ...
    public void testGetCurrentPoint() {
        Assert.assertNull(g.getCurrentPoint());
        g.moveTo(10, 20);
        Assert.assertEquals(new Point2D.Float(10, 20), g.getCurrentPoint());
        g.lineTo(30, 40);
        Assert.assertEquals(new Point2D.Float(30, 40), g.getCurrentPoint());
        g.quadTo(50, 60, 70, 80);
        Assert.assertEquals(new Point2D.Float(70, 80), g.getCurrentPoint());
        g.curveTo(90, 100, 110, 120, 130, 140);
        Assert.assertEquals(new Point2D.Float(130, 140), g.getCurrentPoint());
        g.closePath();
        Assert.assertEquals(new Point2D.Float(10, 20), g.getCurrentPoint());
        g.moveTo(150, 160);
        Assert.assertEquals(new Point2D.Float(150, 160), g.getCurrentPoint());
        g.lineTo(170, 180);
        Assert.assertEquals(new Point2D.Float(170, 180), g.getCurrentPoint());
        g.closePath();
        Assert.assertEquals(new Point2D.Float(150, 160), g.getCurrentPoint());
    }
    ...
    public void testCreateTransformedShape() {
        for (float[][][] element : shapes2) {
            ...
            Assert.assertTrue(Tools.Shape.equals(dst1, dst2, 0.0));
            Assert.assertTrue(Tools.Shape.equals(dst1, dst2, 0.0f));
        }
    }
