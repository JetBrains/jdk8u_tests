...
//        filterImage = createFilter("^(cubic).*([.]ico)$", "(.*)((affine)|(flat)|(bounds))(.*)");
    ...
    public void testGetFlatnessSq1() {
        for (double[] element : curves3) {
            ...
            Assert.assertEquals(cubicToStr(curve), element[8], curve.getFlatnessSq(), 0.0);
        }
    }
    ...
    public void testGetFlatness1() {
        for (double[] element : curves3) {
            CubicCurve2D curve = new CubicCurve2D.Double();
            curve.setCurve(element, 0);
            Assert.assertEquals(cubicToStr(curve), Math.sqrt(element[8]), curve.getFlatness(), 0.0);
        }
    }
    ...
    public void testSolveCubic1() {
        for (double[][] element : equations) {
            ...
            Assert.assertEquals(seqn + " roots count", (int)element[1][0], roots);
            for(int j = 0; j < roots; j++) {
                Assert.assertEquals(seqn + " root(" + j + ")", element[1][1 + j], eqn[j], ROOT_DELTA);
            }
        }
    }
    ...
    public void testSolveCubic2() {
        for (double[][] element : equations) {
            ...
            for(int j = 0; j < roots; j++) {
                Assert.assertEquals(seqn + " root(" + j + ")", element[1][1 + j], res[j], ROOT_DELTA);
            }
        }
    }
